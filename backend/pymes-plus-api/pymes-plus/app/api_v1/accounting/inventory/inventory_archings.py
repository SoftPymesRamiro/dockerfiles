...
from ... import api
from ....models import DocumentHeader, InventoryArching
from flask import request, jsonify, abort
from ....exceptions import ValidationError
from ....decorators import authorize


@api.route('/inventory_archings/search', methods=['GET'])
@authorize('inventaryArching', 'r')
def get_inventory_arching_by_search():
    """
    @api {get} /inventory_archings/search Search Inventory Archings
    @apiName Search
    @apiGroup Inventory.Arching
    @apiDescription Allow obtain documents type inventory arching according to params
    @apiParam {String} short_word="AQ" identifier by document type
    ...
    response = InventoryArching.export_data(response)
    return jsonify(response)


@api.route('/inventory_archings/<int:id_inventory_arching>', methods=['GET'])
@authorize('inventaryArching', 'r')
def get_inventory_arching(id_inventory_arching):
    """
    @api {get} /inventory_archings/inventoryArchingId Get Inventory Arching
    @apiName Get
    @apiGroup Inventory.Arching
    @apiDescription Allow obtain documents type inventory arching according to identifier
    @apiParam {Number} inventoryArchingId document identifier
    ...
    @apiErrorExample {json} Find error
        HTTP/1.1 500 Internal Server Error
    """
    response = InventoryArching.get_by_id(id_inventory_arching)
    if response:
        # Exporto el consumo interno en formato json
        response = InventoryArching.export_data(response)
        return jsonify(response)
...
@api.route('/inventory_archings/', methods=['POST'])
@authorize('inventaryArching', 'c')
def post_inventory_arching():
    """
    @api {post} /inventory_archings/ Create a New Inventory Arching
    @apiName New
    @apiGroup Inventory.Arching
    @apiParam {Number} inventoryArchingId Inventory arching identifier
    @apiDescription Create a inventory arching document
    ...
    if short_word is None:
        raise ValidationError("Invalid params")
    document_header_id, documentNumber = InventoryArching.save_inventory_arching(data, short_word)
    response = jsonify({'id': document_header_id, 'documentNumber': documentNumber})
    response.status_code = 201
    return response


@api.route('/inventory_archings/<int:id_inventory_arching>', methods=['PUT'])
@authorize('inventaryArching', 'u')
def put_inventory_arching(id_inventory_arching):
    """
    @api {put} /inventory_archings/inventoryArchingId Update a Inventory Arching
    @apiName Update
    @apiGroup Inventory.Arching
    @apiParam {Number} inventoryArchingId Inventory arching identifier
    @apiDescription Update a inventory arching document according to id
    @apiParamExample {json} Input
        {
        "sourceDocumentHeaderId": null,
        "documentNumber": "0000000019",
    ...
    """
    data = request.json
    response = InventoryArching.update_inventory_arching(id_inventory_arching, data)
    return response


@api.route('/inventory_archings/<int:id_inventory_arching>', methods=['DELETE'])
@authorize('inventaryArching', 'd')
def delete_inventory_arching(id_inventory_arching):
    """
    @api {delete} /inventory_archings/inventoryArchingId Remove Inventory Arching
    @apiName Delete
    @apiGroup Inventory.Arching
    @apiParam {Number} inventoryArchingId Inventory arching identifier
    @apiDescription Delete a inventory arching document according to id
    ...
    @apiErrorExample {json} Delete error
       HTTP/1.1 500 Internal Server Error
    """
    response = InventoryArching.delete_inventory_arching(id_inventory_arching)
    return response


@api.route('/inventory_archings/', methods=['GET'])
@authorize('inventaryArching', 'r')
def get_all_inventory_arching():
    """
    @api {get}  /inventory_archings/ xAll Inventory Archings
    @apiName All
    @apiGroup Inventory.Arching
    @apiDescription Allow obtain all documents with type inventory arching
    ...
    HTTP/1.1 500 Internal Server Error
    """
    response = InventoryArching.get_all()
    return response


@api.route('/inventory_archings/<int:id_inventory_arching>/preview', methods=['GET'])
@authorize('inventaryArching', 'r')
def get_inventory_arching_preview(id_inventory_arching):
    """
        @api {get}  /inventory_archings/inventoryArchingId/preview Preview Inventory Arching
        @apiName Preview
        @apiGroup Inventory.Arching
        @apiDescription Returns preview of Inventory Arching
        @apiParam {Number} inventoryArchingId Inventory Arching identifier
    ...
    response = InventoryArching.get_document_preview(id_inventory_arching, format_type)
    if response is None:
        abort(404)
