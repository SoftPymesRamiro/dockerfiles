...
from ....reports import InvoiceProfessionalServicesPreview
from ....reports import InvoiceProfessionalServicesPreviewM
from ....reports import InvoiceProfessionalServicesPreviewF

class SaleInvoiceProfessionalServices(DocumentHeader):
    """
    ...
    def save_sale_professional_services(data, short_word):
        ...
        try:
            ...
            source_id = session.query(DocumentType.documentTypeId) \
                .filter(DocumentType.shortWord == 'FS').scalar()
            ...
            SaleInvoiceProfessionalServices.save_accounting(document_header, document_details, payment_recipt)
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        # Proceso de contabilizacion
        from ....models import SaleInvoiceProfessionalServiceAccounting
        accounting = SaleInvoiceProfessionalServiceAccounting(document_header)
    ...
    def update_sale_professional_services(id_sale_professional_services, data):
        ...
        try:
            ...
            document_header = DocumentHeader.get_by_id(id_sale_professional_services)
            ...
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = SaleInvoiceProfessionalServices.update_document_details(document_header, data, dds,
                                                                   document_detail_list,
                                                                   id_sale_professional_services)
            # Consulta el payment Receipt
            receipt_saved = PaymentReceipt.get_receipt_by_document_identifier(id_sale_professional_services)
            ...
            if len(payment_receipt) > 0:
                payment_receipt = SaleItem.update_payment_receipt(document_header, data, receipt_saved,
                                                                 payment_receipt, id_sale_professional_services)
            else:
                payment_receipt_saved = session.query(PaymentReceipt.paymentReceiptId) \
                    .filter(PaymentReceipt.documentHeaderId == id_sale_professional_services)
            ...
            SaleInvoiceProfessionalServices.update_accounting(id_sale_professional_services, document_header,
    ...
    def update_accounting(id_sale, dh, all_document_details, payment_receipt):
        ...
        :return: None
        """
        accounting_records = SaleInvoiceProfessionalServices.get_accounting_by_invoice_id(id_sale)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            SaleInvoiceProfessionalServices.save_accounting(dh, all_document_details, payment_receipt)
    ...
    def get_document_preview(document_header_id, format_type='P', document_type='D', invima=0, copy_or_original=1):
        ...
        try:
            if document_type == 'D':
                ...
                if format_type == 'P':
                    return InvoiceProfessionalServicesPreview.make_preview_pdf(formatted_data)
                elif format_type == 'M':
                    return InvoiceProfessionalServicesPreviewM.make_preview_pdf(formatted_data)
                elif format_type == 'F':
                    return InvoiceProfessionalServicesPreviewF.make_preview_pdf(formatted_data)
