...
from ....reports import ClientsNotesPreview
from ....reports import ClientsNotesPreviewM
from datetime import timedelta
from ....utils.converters import format_cc_or_nit


class CustomerNote(DocumentHeader):
    """CustomerNote as a public model class.
    note::
    """
    ...
    def export_data(data):
        ...
        :return: customer note in JSON format
        ...
        return {
            ...
            'consumptionTaxPUC': None if data.consumptionTaxPUC is None else data.consumptionTaxPUC.export_data(),
    ...
    def get_by_id(id_note):
        """
        Allow obtain customer note for to give a identifier
        :param id: identifier by purchase item to obtain
        :return: purchase order in JSON format
        """
        provider_note = session.query(DocumentHeader).filter(
            DocumentHeader.documentHeaderId == id_note
        ).first()

        return provider_note

    @staticmethod
    def save_customer_note(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            if not (short_word in ("ND", "NC")):
                abort(400)
            # Crea el objeto document_header
            document_header = DocumentHeader()
            document_header.import_data(data)

            # Importación del documentdetails
            document_detail_list = data['documentDetails']
            document_details = CustomerNote.import_details(document_detail_list)

            # Obtiene el document_Type_id ya que solo esta recibiendo el short_word
            ...
            # Hace flush al document_header para obtener el id y utilizarlo al guardar los detalles
            document_header_id = document_header.save()
            # Guarda los detalles del documento
            CustomerNote.save_document_details(document_header, document_header_id, document_details)
            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            CustomerNote.save_exchange_rate(document_header)
            # Proceso que contabiliza
            CustomerNote.save_accounting(document_header, document_details, None)
            # Persistencia a base de datos
            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            CustomerNote.validate_state_document(document_header, data, document_details)
            # session.expire_all()
            customer_note = session.query(DocumentHeader).get(document_header_id)
            customer_note = DocumentHeader.export_data(customer_note)

            return document_header_id, customer_note['documentNumber']
    ...
    @staticmethod
    def update_customer_note(id_customer_note, data):
        """
        Allow updater a purchase item according to data and its identifier
        :param id_customer_note: identifier by purchase order to update
        :param data: information by new purchase order
        :exception: An error occurs when update not performance
        :return: purchase order update in JSON format
        """
        try:
            # Valida que el id enviado en la url sea el mismo enviado en el data
            if id_customer_note != data['documentHeaderId']:
                raise ValidationError('DocumentHeaderId is different to payload')

            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_customer_note)

            # Valida si existe el document_header
            if document_header is None:
                raise ValidationError('DocumentHeaderId does not exist')
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_customer_note)

            document_detail_list = data['documentDetails']

            # Actualiza detalles
            all_document_details = CustomerNote.update_document_details(document_header, data, dds,
                                                                        document_detail_list,
                                                                        id_customer_note)
            # Genera la contabilidad
            CustomerNote.update_accounting(id_customer_note, document_header, all_document_details, None)

            session.commit()

            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            CustomerNote.validate_state_document(document_header, data, all_document_details)
    ...
    @staticmethod
    def detele_customer_note(id_customer_note):
        """
        Allow delete a purchase item according its identifier
        :param id_customer_note: Id item
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_customer_note).first()
            if document_header is None:
                response = jsonify({'code': 404, 'message': 'Customer note Not Found'})
            ...
            accounting_records = session.query(AccountingRecord) \
                .filter(or_(AccountingRecord.documentHeaderId == id_customer_note,
                            AccountingRecord.crossDocumentHeaderId == id_customer_note)).delete()
            session.flush()
    ...
    @staticmethod
    def get_accounting_by_customer_note_id(customer_note_id):
        """
        Allow get accounting by purchase itemn id
        :param customer_note_id: purchase item id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == customer_note_id).all()
    ...
    def delete_serials(document_detail_id):
        ...
        :param id_provider_note: id purchase item
        :return: None
        """
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        def validate_accounts(ret_value):
            ...
            if not (c == d):
                raise InternalServerError('Descuadre {0} - {1}'.format(str(d), str(c)))

            return ret_value

        # Proceso de contabilizacion
        ret_value = []
        if document_header.documentType.shortWord == "NC":
            from ....models import CustomerNoteAccountingNC
            accounting = CustomerNoteAccountingNC(document_header)
        ...
        elif document_header.documentType.shortWord == "ND":
            from ....models import CustomerNoteAccountingND
            accounting = CustomerNoteAccountingND(document_header)
            # Retorna lista con los resgistros de contabilizacion
            ret_value = accounting.do_account(document_header, document_details, payment_receipt)
    ...
    @staticmethod
    def update_accounting(id_provider_note, dh, all_document_details, payment_receipt):
        """
        ...
        :return: None
        """
        accounting_records = CustomerNote.get_accounting_by_customer_note_id(id_provider_note)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            CustomerNote.save_accounting(dh, all_document_details, payment_receipt)
    ...
    @staticmethod
    def update_document_details(dh, data, dds, document_detail_list, id_customer_note):
        """
        ...
        :param id_customer_note: id purchase item
        ...
        for d in document_detail_list:
            if 'documentDetailId' not in d:
                ...
                new_detail.documentHeaderId = id_customer_note
        ...
        for dd in all_document_details:
            ...
            if dd.listSerials is not None and (not dd.sourceDocumentDetail):
                CustomerNote.delete_serials(dd.documentDetailId)  # elimina los seriales
                for s in dd.listSerials:
                    CustomerNote.save_serial(s, dd)  # crea nuevamente los seriales
    ...
    @staticmethod
    def delete_customer_note(id_customer_note):
        """
            Allow delete a customer note according its identifier
            :param id_customer_note: Id customer note
            :return: None if not found the document, {} if delete is successful
            :exception: An error occurs when delete the document
            """
        try:
        ...
        try:
            if document_type == 'D':
                ...
                formatted_data = {
                    ...
                    'business_agent': 'None' if preview_data.businessAgent is None
                    else "{0} {1} {2} {3} - {4}".format(
                    ...
                    'sourceData': preview_data.sourceDocumentHeader,
                    ...
                    'shipZipCode': '' if preview_data.shipZipCode is None else preview_data.shipZipCode,
                    ...
                    'sourceControlNumber': preview_data.sourceDocumentHeader.controlNumber,
                    ...
                    'shortWord': preview_data.source.shortWord,
                }

                if format_type == 'P':
                    return ClientsNotesPreview.make_preview_pdf(formatted_data)
                elif format_type == 'M':
                    return ClientsNotesPreviewM.make_preview_pdf(formatted_data)
