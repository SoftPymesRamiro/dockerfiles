...
class SaleInvoiceGlobalRemission(DocumentHeader):
    ...
    @staticmethod
    def save_sale_invoice_global_remission(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            document_details = SaleInvoiceGlobalRemission.import_details(document_detail_list)
            ...
            SaleInvoiceGlobalRemission.save_document_details(document_header, document_header_id, document_details)
            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            SaleInvoiceGlobalRemission.save_exchange_rate(document_header)
            payment_recipt = None
            if 'paymentReceipt' in data and data['paymentReceipt']:
                ...
                payment_recipt = SaleInvoiceGlobalRemission.import_payment_receipt(payment_recipt)
            ...
            SaleInvoiceGlobalRemission.save_accounting(document_header, document_details, payment_recipt)
            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            SaleInvoiceGlobalRemission.validate_state_document(document_header, data, document_details)
    ...
    @staticmethod
    def update_sale_invoice_remission(id_invoice_gremission, data):
        """
        ...
        try:
            ...
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = SaleInvoiceGlobalRemission.update_document_details(document_header, data,
                                                                                dds, document_detail_list, id_invoice_gremission)
            ...
            SaleInvoiceGlobalRemission.update_accounting(id_invoice_gremission, document_header, all_document_details, payment_receipt)
            session.commit()

            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            SaleInvoiceGlobalRemission.validate_state_document(document_header, data, all_document_details)
    ...
    def update_accounting(id_sale_item, dh, all_document_details, payment_receipt):
        ...
        :return: None
        """
        accounting_records = SaleInvoiceGlobalRemission.get_accounting_by_sale_id(id_sale_item)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records is not None:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            SaleInvoiceGlobalRemission.save_accounting(dh, all_document_details, payment_receipt)
    ...
    @staticmethod
    def delete_sale_invoice_global_remission(id_invoice_gremission):
        """
        ...
        for dd in document_details:
            ...
            if dd.listSerials is not None:
                for s in dd.listSerials:
                    SaleInvoiceGlobalRemission.save_serials(s, dd)
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        # Proceso de contabilizacion
        from ....models import SaleInvoiceGlobalRemissionAccounting
        accounting = SaleInvoiceGlobalRemissionAccounting(document_header)
    ...
    @staticmethod
    def get_invoice_g_remission_preview(document_header_id, format_type='P', document_type='D', invima=0, copy_or_original=1):
        """
