...
from ....models import DocumentHeader, DocumentType, DocumentDetail, Customer, PaymentReceipt,\
    ...
    EconomicActivity, EconomicActivityPercentage
...
from ....reports import SaleAIUPreview
from ....reports import SaleAIUPreviewM
from ....reports import SaleAIUPreviewF

from datetime import timedelta
from ....utils.converters import format_cc_or_nit

class SaleAIU(DocumentHeader):
    """
    SaleAIU as a public model class.
    note::
    """
    @staticmethod
    ...
    @staticmethod
    def save_sale_aiu(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            if 'paymentReceipt' in data and data['paymentReceipt']:
                ...
                payment_recipt = SaleAIU.import_payment_receipt(payment_recipt)
                # Guarda el payment Receipt con los payment details
                SaleAIU.save_payment_receipt(document_header, payment_recipt)

            SaleAIU.save_accounting(document_header, document_details, payment_recipt)
            session.commit()

            sale_aiu = session.query(DocumentHeader).get(document_header_id)
            sale_aiu = DocumentHeader.export_data(sale_aiu)

            return document_header_id, sale_aiu['documentNumber']
    ...
    @staticmethod
    def update_sale_aiu(id_sale_aiu, data):
        """
        Allow updater a purchase order according to data and its identifier
        :param id_sale_aiu: identifier by purchase order to update
        ...
        try:
            ...
            if id_sale_aiu != data['documentHeaderId']:
                abort(400)

            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_sale_aiu)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_sale_aiu)
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = SaleAIU.update_document_details(document_header, data, dds,
                                                                        document_detail_list,
                                                                        id_sale_aiu)
            # Consulta el payment Receipt
            receipt_saved = PaymentReceipt.get_receipt_by_document_identifier(id_sale_aiu)
            payment_receipt = data['paymentReceipt']
            if len(payment_receipt) > 0:
                payment_receipt = SaleAIU.update_payment_receipt(document_header, data, receipt_saved,
                                                                      payment_receipt, id_sale_aiu)
            else:
                payment_receipt_saved = session.query(PaymentReceipt.paymentReceiptId) \
                    .filter(PaymentReceipt.documentHeaderId == id_sale_aiu)
            ...
            SaleAIU.update_accounting(id_sale_aiu, document_header, all_document_details, payment_receipt)
            session.commit()
    ...
    @staticmethod
    def delete_sale_aiu(id_sale_aiu):
        """
            Allow delete a purchase order according its identifier
            :param id_sale_aiu: Id purchase order
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_sale_aiu).first()
            if document_header is None:
                return None

            accounting_records = session.query(AccountingRecord)\
                .filter(or_(AccountingRecord.documentHeaderId == id_sale_aiu,
                            AccountingRecord.crossDocumentHeaderId == id_sale_aiu)).delete()
            session.flush()
    ...
    @staticmethod
    def get_document_preview(document_header_id, format_type='P',invima=0, copy_or_original='1'):
        """
        ...
        try:
            ...
            if format_type == 'P':
                return SaleAIUPreview.make_preview_pdf(formatted_data)
            elif format_type == 'M':
                return SaleAIUPreviewM.make_preview_pdf(formatted_data)
            elif format_type == 'F':
                return SaleAIUPreviewF.make_preview_pdf(formatted_data)
    ...
    @staticmethod
    def get_accounting_by_sale_aiu_id(sale_aiu_id):
        """
        Allow get accounting by purchase remissionn id
        :param sale_aiu_id: purchase remission id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == sale_aiu_id).all()
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        from ....models import SaleAIUAccounting
        accounting = SaleAIUAccounting(document_header)
    ...
    @staticmethod
    def update_accounting(id_sale_aiu, dh, all_document_details,payment_receipt):
        """
        ...
        :return: None
        """
        accounting_records = SaleAIU.get_accounting_by_sale_aiu_id(id_sale_aiu)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            SaleAIU.save_accounting(dh, all_document_details,payment_receipt)
    ...
    @staticmethod
    def update_payment_receipt(document_header, data, payment_receipt_old, payment_receipt_new, id_sale_aiu):
        """
        ...
        :param id_sale_aiu: id advance third
    ...
    def get_accounting_by_sale_aiu_id(sale_aiu_id):
        ...
        :param sale_aiu_id: purchase item id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
