...
class SaleInvoiceThirdPartyAccounting(object):
            ...
            if self.total_records > 0:
                ...
                for detail in document_details:
                    ...
                    if (not (detail.sourceDocumentDetailId is not None
                        ...
                        if detail.sourceDocumentDetail is not None and \
                            ...
                            if detail.sourceDocumentDetail.documentHeader.destinyWarehouse is not None \
                                ...
                                if detail.detailWarehouse is not None \
                                    ...
                                    self.ret_value.append(self.cost_inventory(document_header, detail, "D"))
                        # // Ingreso
                        if not(detail.providerId is None) and not(detail.provider is None):
                            self.ret_value.append(
                                self.income_for_others(document_header, detail, self.round_decimals, "C", detail.value))
                        else:
                            self.ret_value.append(
                                self.income_inventory(document_header, detail, self.round_decimals, "C"))
                        ...
                        if not(detail.baseValue is None) and detail.baseValue > 0 \
                                and not(detail.iva is None) and detail.iva > 0 and \
                                (detail.provider is None and detail.providerId is None):
                        ...
                        if detail.withholdingTax and detail.withholdingTax > 0 and detail.value > 0\
                                and (detail.provider is None and detail.providerId is None):
