...
from ....models import DocumentHeader, DocumentType, DocumentDetail, Provider,\
    ExchangeRate, DefaultValue, AccountingRecord, Serial, SerialDetail, IVAType
...
from ....reports import SaleRemissionPreview
from ....reports import SaleRemissionPreviewM
from ....reports import SaleRemissionPreviewF
from ....utils.converters import format_cc_or_nit

class SaleRemission(DocumentHeader):
    """
    SaleRemission is a abstract document which is used to stand in for a
    ...
    def get_by_id(id):
        ...
        """
        sale_remission = session.query(DocumentHeader).get(id)
        return sale_remission

    @staticmethod
    def save_sale_remission(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            document_details = SaleRemission.import_details(document_detail_list)
            ...
            SaleRemission.save_document_details(document_header, document_header_id, document_details)

            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            SaleRemission.save_exchange_rate(document_header)

            # Proceso de contabilizacion
            SaleRemission.save_accounting(document_header, document_details)

            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            # SaleRemission.validate_state_document(document_header, data, document_details)
            # session.expire_all()
            sale_remission = session.query(DocumentHeader).get(document_header_id)
            sale_remission = DocumentHeader.export_data(sale_remission)

            return document_header_id, sale_remission['documentNumber']
    ...
    @staticmethod
    def update_sale_remission(id_sale_remission, data):
        """
        Allow updater a purchase remission according to data and its identifier
        :param id_sale_remission: identifier by sale remission to update
        ...
        try:
            ...
            if id_sale_remission != data['documentHeaderId']:
                abort(400)

            # Obtiene el documentheader por el id
            dh = DocumentHeader.get_by_id(id_sale_remission)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_sale_remission)

            # Actuliza seriales
            SaleRemission.update_serials(id_sale_remission)

            document_detail_list = data['documentDetails']

            # Actualiza detalles
            all_document_details = SaleRemission.update_document_details(dh, data, dds, document_detail_list,
                                                                             id_sale_remission)
            # Genera la contabilidad
            SaleRemission.update_accounting(id_sale_remission, dh, all_document_details)

            session.commit()

            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            SaleRemission.validate_state_document(dh, data, all_document_details)
    ...
    def delete_sale_remission(id_sale_remission):
        ...
        :param id_sale_remission: Id remission
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_sale_remission).first()
            if document_header is None:
                response = jsonify({'code': 404, 'message': 'Sale remission Not Found'})
                response.status_code = 404
                return response
            # Consulta los accounting records para eliminarlos
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == id_sale_remission).all()
    ...
    @staticmethod
    def get_accounting_by_sale_remission_id(sale_remission_id):
        """
        Allow get accounting by purchase remissionn id
        :param sale_remission_id: purchase remission id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == sale_remission_id).all()
    ...
    @staticmethod
    def update_serials(id_sale_remission):
        """
        ...
        serial_details = SerialDetail.get_serial_detail_by_document_header_id(id_sale_remission)
        [sd.delete() for sd in serial_details]
        # Elimina seriales de ultimo
        [s.delete() for s in Serial.get_serial_by_document_header_id(id_sale_remission)]
    ...
    def save_accounting(document_header, document_details):
        ...
        """
        # Proceso de contabilizacion
        from ....models import SaleRemissionAccounting
        accounting = SaleRemissionAccounting(document_header)
    ...
    @staticmethod
    def update_accounting(id_sale_remission, dh, all_document_details):
        """
        ...
        :return: None
        """
        accounting_records = SaleRemission.get_accounting_by_sale_remission_id(id_sale_remission)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records is not None:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            SaleRemission.save_accounting(dh, all_document_details)
        ...
        for dd in document_details:
            ...
            if dd.listSerials is not None:
                for s in dd.listSerials:
                    SaleRemission.save_serials(s, dd)
    ...
    @staticmethod
    def get_document_preview(document_header_id, format_type='P', document_type='D', invima =0, not_value='1'):
        ...
        try:
            if document_type == 'D':
                ...
                formatted_data = {
                    ...
                    else preview_data.branch.company.name.upper(),
                    ...
                    'regimen': ' ' if not regimen else 'RÉGIMEN {0}'.format(
                                                                        'COMÚN' if regimen.name.upper() == 'COMUN'
                    ...
                    'employee_zone': ' ' if not preview_data.employee  or preview_data.employee.zipCode is None
                    else preview_data.employee.zipCode,
                    ...
                    'business_agent_zone': ' ' if not preview_data.businessAgent  or preview_data.businessAgent.billZipCode is None
                    else preview_data.businessAgent.billZipCode,
                    ...
                    'percentage_renta': preview_data.branch.economicActivity.percentage,
                    'invima': invima,
                    'not_value': not_value,
                ...
                if format_type == 'P':
                    return SaleRemissionPreview.make_preview_pdf(formatted_data)
                elif format_type == 'M':
                    return SaleRemissionPreviewM.make_preview_pdf(formatted_data)
                elif format_type == 'F':
                    return SaleRemissionPreviewF.make_preview_pdf(formatted_data)
