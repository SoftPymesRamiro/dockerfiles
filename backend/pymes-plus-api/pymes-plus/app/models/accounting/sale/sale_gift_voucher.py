...
from ....models import DocumentHeader, DocumentType, DocumentDetail, Customer,\
    ...
    PaymentReceipt, PaymentDetail, PaymentMethod, PaymentAccounting
...
from ....reports import GiftVoucherPreview

class GiftVoucher(DocumentHeader):
    """
    GiftVoucher is a abstract document which is used to stand in for a
    ...
    def get_by_id(id):
        ...
        """
        gift_voucher = session.query(DocumentHeader).get(id)
        return gift_voucher

    @staticmethod
    def save_gift_voucher(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            GiftVoucher.save_exchange_rate(document_header)
            ...
            if 'paymentReceipt' in data and data['paymentReceipt']:
                ...
                payment_recipt = GiftVoucher.import_payment_receipt(payment_recipt)
            ...
            GiftVoucher.save_accounting(document_header, payment_recipt)
            session.commit()
            gift_voucher = session.query(DocumentHeader).get(document_header_id)
            gift_voucher = DocumentHeader.export_data(gift_voucher)

            return document_header_id, gift_voucher['documentNumber']
    ...
    @staticmethod
    def update_gift_voucher(id_gift_voucher, data):
        """
        Allow updater a sale item according to data and its identifier
        :param id_gift_voucher: identifier by sale remission to update
        ...
        try:
            ...
            if id_gift_voucher != data['documentHeaderId']:
                abort(400)
            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_gift_voucher)
            ...
            receipt_saved = PaymentReceipt.get_receipt_by_document_identifier(id_gift_voucher)
            payment_receipt = data['paymentReceipt']
            if len(payment_receipt) > 0:
                payment_receipt = PaymentAccounting.update_payment(document_header, receipt_saved, payment_receipt)
            else:
                payment_receipt_saved = session.query(PaymentReceipt.paymentReceiptId) \
                    .filter(PaymentReceipt.documentHeaderId == id_gift_voucher)
            ...
            GiftVoucher.update_accounting(id_gift_voucher, document_header, payment_receipt)
            session.commit()
    ...
    @staticmethod
    def delete_gift_voucher(id_gift_voucher):
        """
        Allow delete a sale item according its identifier
        :param id_gift_voucher: Id remission
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_gift_voucher).first()
            if document_header is None:
            ...
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == id_gift_voucher).all()
    ...
    @staticmethod
    def get_accounting_by_gift_voucher_id(gift_voucher_id):
        """
        Allow get accounting by sale itemn id
        :param gift_voucher_id: sale item id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == gift_voucher_id).all()
    ...
    def import_payment_receipt(data):
        ...
        for data_payment in data['paymentDetails']:
            ...
            payment_type_id = session.query(PaymentMethod.paymentMethodId).filter(
                ...
                (True if 'paymentMethod' not in data_payment else data_payment['paymentMethod']['code']))\
                .scalar()
    ...
    def save_accounting(document_header, payment_receipt):
        ...
        from ....models import GiftVoucherAccounting
        accounting = GiftVoucherAccounting(document_header)
    ...
    @staticmethod
    def update_accounting(id_gift_voucher, dh, payment_receipt):
        """
        Allow update accounting records
        :param id_gift_voucher: id sale item
        :param dh: document header object
        :param all_document_details: document details list
        :return: None
        """
        accounting_records = GiftVoucher.get_accounting_by_gift_voucher_id(id_gift_voucher)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records is not None:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            GiftVoucher.save_accounting(dh, payment_receipt)
    ...
    def get_document_preview(document_header_id, format_type='P', document_type='D'):
        ...
        try:
            if document_type == 'D':
                ...
                if format_type == 'P':
                    return GiftVoucherPreview.make_preview_pdf(formatted_data)
