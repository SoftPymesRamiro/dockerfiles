...
from ....models import DocumentHeader, DocumentType, DocumentDetail, Provider,\
    ExchangeRate, DefaultValue, AccountingRecord, Serial, SerialDetail, IVAType, Image
...
from ....reports import SaleOrderPreview
from ....reports import SaleOrderPreviewM
from ....reports import SaleOrderPreviewF
from ....reports import SaleOrderPreviewT
from ....utils.converters import format_cc_or_nit

class SaleOrder(DocumentHeader):
    """
    SaleOrder is a abstract document which is used to stand in for a
    ...
    def get_by_id(id):
        ...
        """
        sale_order = session.query(DocumentHeader).get(id)
        return sale_order

    @staticmethod
    def save_sale_order(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            document_details = SaleOrder.import_details(document_detail_list)
            ...
            SaleOrder.save_document_details(document_header, document_header_id, document_details)

            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            SaleOrder.save_exchange_rate(document_header)

            # Proceso de contabilizacion
            SaleOrder.save_accounting(document_header, document_details)

            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            # SaleOrder.validate_state_document(document_header, data, document_details)
            # session.expire_all()
            sale_order = session.query(DocumentHeader).get(document_header_id)
            sale_order = DocumentHeader.export_data(sale_order)

            return document_header_id, sale_order['documentNumber']
    ...
    @staticmethod
    def update_sale_order(id_sale_order, data):
        """
        Allow updater a purchase remission according to data and its identifier
        :param id_sale_order: identifier by sale remission to update
        ...
        try:
            ...
            if id_sale_order != data['documentHeaderId']:
                abort(400)

            # Obtiene el documentheader por el id
            dh = DocumentHeader.get_by_id(id_sale_order)
            ...
            dh.import_data(data)
            if data['customerId']:
                dh.provider = session.query(Provider).get(data['customerId'])

            # Actualiza el documentHeader
            dh.update()

            # Consulta los detalles del documentheader
            dds = DocumentDetail.get_document_details_by_document_header_id(id_sale_order)

            # Actuliza seriales
            SaleOrder.update_serials(id_sale_order)

            document_detail_list = data['documentDetails']

            # Actualiza detalles
            all_document_details = SaleOrder.update_document_details(dh, data, dds, document_detail_list,
                                                                             id_sale_order)
            # Genera la contabilidad
            SaleOrder.update_accounting(id_sale_order, dh, all_document_details)

            session.commit()

            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            SaleOrder.validate_state_document(dh, data, all_document_details)
    ...
    def delete_sale_order(id_sale_order):
        ...
        :param id_sale_order: Id remission
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_sale_order).first()
            if document_header is None:
                response = jsonify({'code': 404, 'message': 'Sale order Not Found'})
                response.status_code = 404
                return response
            # Consulta los accounting records para eliminarlos
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == id_sale_order).all()
    ...
    @staticmethod
    def get_accounting_by_sale_order_id(sale_order_id):
        """
        Allow get accounting by purchase remissionn id
        :param sale_order_id: purchase remission id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == sale_order_id).all()
    ...
    @staticmethod
    def update_serials(id_sale_order):
        """
        Allow update serials from document
        :param id_sale_order: id purchase remission
        :return: None
        """
        # Actuliza seriales (los elimina primero)
        # Elimina serial details primero
        serial_details = SerialDetail.get_serial_detail_by_document_header_id(id_sale_order)
        [sd.delete() for sd in serial_details]
        # Elimina seriales de ultimo
        [s.delete() for s in Serial.get_serial_by_document_header_id(id_sale_order)]
    ...
    def save_accounting(document_header, document_details):
        ...
        """
        # Proceso de contabilizacion
        # from ....models import SaleOrderAccounting
        # accounting = SaleOrderAccounting(document_header)
    ...
    @staticmethod
    def update_accounting(id_sale_order, dh, all_document_details):
        """
        ...
        :return: None
        """
        accounting_records = SaleOrder.get_accounting_by_sale_order_id(id_sale_order)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records is not None:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            SaleOrder.save_accounting(dh, all_document_details)
    ...
    @staticmethod
    def update_document_details(dh, data, dds, document_detail_list, id_sale_order):
        """
        ...
        for d in document_detail_list:
            if 'documentDetailId' not in d:
                ...
                new_detail.documentHeaderId = id_sale_order
    ...
    @staticmethod
    def get_document_preview(document_header_id, format_type='P', document_type='D', invima='0', not_value='0'):
        """
        ...
        try:
            if document_type == 'D':
                ...
                formatted_data = {
                    ...
                    # 'business_agent': preview_data.businessAgent,
                ...
                if format_type == 'P':
                    return SaleOrderPreview.make_preview_pdf(formatted_data)
                elif format_type == 'M':
                    return SaleOrderPreviewM.make_preview_pdf(formatted_data)
                elif format_type == 'F':
                    return SaleOrderPreviewF.make_preview_pdf(formatted_data)
                elif format_type == 'T':
                    return SaleOrderPreviewT.make_preview_pdf(formatted_data)
