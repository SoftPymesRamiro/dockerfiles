...
from ....models import  DefaultValue, Image
...
from ....reports import SaleQuotationPreview
from ....reports import SaleQuotationPreviewM
from ....reports import SaleQuotationPreviewF

class SaleQuotation(DocumentHeader):
    """
    SaleQuotation as a public model class.
    """
    @staticmethod
    ...
    def get_by_id(id):
        ...
        """
        po = session.query(DocumentHeader).filter(DocumentHeader.documentHeaderId == id, DocumentDetail.balance != 0).first()
        return po

    @staticmethod
    def save_sale_quotation(data, short_word):
        """
        ...
        try:
            ...
            # session.expire_all()
            sale_quotation = session.query(DocumentHeader).get(document_header_id)
            sale_quotation = DocumentHeader.export_data(sale_quotation)

            return document_header_id, sale_quotation['documentNumber']
    ...
    @staticmethod
    def update_sale_quotation(id_sale_quotation, data):
        """
        Allow updater a purchase order according to data and its identifier
        :param id_sale_quotation: identifier by purchase order to update
        ...
        try:
            ...
            if id_sale_quotation != data['documentHeaderId']:
                abort(400)

            # Obtiene el documentheader por el id
            dh = DocumentHeader.get_by_id(id_sale_quotation)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_sale_quotation)
            ...
            for d in document_detail_list:
                if 'documentDetailId' not in d:
                    ...
                    new_detail.documentHeaderId = id_sale_quotation
    ...
    def delete_sale_quotation(id_sale_quotation):
        """
            ...
            :param id_sale_quotation: Id purchase order
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_sale_quotation).first()
    ...
    def get_document_preview(document_header_id, format_type='P', invima=0):
        ...
        try:
            ...
            if format_type == 'P':
                return SaleQuotationPreview.make_preview_pdf(formatted_data)
            elif format_type == 'M':
                return SaleQuotationPreviewM.make_preview_pdf(formatted_data)
            elif format_type == 'F':
                return SaleQuotationPreviewF.make_preview_pdf(formatted_data)
