...
from ....models import DocumentHeader, DocumentType, DocumentDetail, Customer,PaymentReceipt,\
    ...
    AccountingRecord,IVAType, Image, GeneralParameter, EconomicActivityPercentage, \
    PaymentAccounting
...
from ....reports import InvoiceInversionsPreview
from ....reports import InvoiceInversionsPreview_M
from ....reports import InvoiceInversionsPreview_F
from datetime import timedelta
from ....utils.converters import format_cc_or_nit

class SaleInvoiceInversion(DocumentHeader):
    """
    ...
    def export_data(data):
        ...
        return {
            ...
            'documentDetails': None if data.documentDetails is None else [dd.export_data_concepts() for dd in data.documentDetails],
    ...
    @staticmethod
    def save_sale_inversion(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            document_details = SaleInvoiceInversion.import_details(document_detail_list)
            ...
            SaleInvoiceInversion.save_document_details(document_header, document_header_id, document_details)
            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            SaleInvoiceInversion.save_exchange_rate(document_header)
            payment_recipt = None
            if 'paymentReceipt' in data and data['paymentReceipt']:
                ...
                payment_recipt = SaleInvoiceInversion.import_payment_receipt(payment_recipt)
            ...
            SaleInvoiceInversion.save_accounting(document_header, document_details, payment_recipt)
            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            SaleInvoiceInversion.validate_state_document(document_header, data, document_details)
    ...
    @staticmethod
    def update_sale_inversion(id_sale_item, data):
        """
        ...
        try:
            ...
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = SaleInvoiceInversion.update_document_details(document_header, data,
            ...
            SaleInvoiceInversion.update_accounting(id_sale_item, document_header, all_document_details, payment_receipt)
            session.commit()

            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            SaleInvoiceInversion.validate_state_document(document_header, data, all_document_details)
    ...
    @staticmethod
    def delete_sale_inversion(id_sale_item):
        """
    ...
    @staticmethod
    def get_accounting_by_sale_inversion_id(sale_item_id):
        """
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        def validate_accounts(ret_value):
            ...
            return ret_value
        # Proceso de contabilizacion
        from ....models import SaleInvoiceInversionAccounting
        accounting = SaleInvoiceInversionAccounting(document_header)
    ...
    def update_accounting(id_sale_item, dh, all_document_details, payment_receipt):
        ...
        :return: None
        """
        accounting_records = SaleInvoiceInversion.get_accounting_by_sale_inversion_id(id_sale_item)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records is not None:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            SaleInvoiceInversion.save_accounting(dh, all_document_details, payment_receipt)
        ...
        try:
            if document_type == 'D':
                ...
                formatted_data = {
                    ...
                    'doc_number': '' if len(preview_data.documentDetails.all()) <= 0
                    else preview_data.documentDetails.all()[0].detailDocument,
                    'name_details': '' if len(preview_data.documentDetails.all()) <= 0
                    else "{0} {1} {2} - {3}".format(
                        "" if preview_data.documentDetails.all()[0].thirdParty.tradeName is None
                        else preview_data.documentDetails.all()[0].thirdParty.tradeName.strip(),
                        "" if preview_data.documentDetails.all()[0].thirdParty.lastName is None
                        else preview_data.documentDetails.all()[0].thirdParty.lastName.strip(),
                        "" if preview_data.documentDetails.all()[0].thirdParty.maidenName is None
                        else preview_data.documentDetails.all()[0].thirdParty.maidenName.strip(),
                        "" if preview_data.documentDetails.all()[0].thirdParty.firstName is None
                        else preview_data.documentDetails.all()[0].thirdParty.firstName.strip()),

                }

                if format_type == 'P':
                    return InvoiceInversionsPreview.make_preview_pdf(formatted_data)
                elif format_type == 'M':
                    return InvoiceInversionsPreview_M.make_preview_pdf(formatted_data)
                elif format_type == 'F':
                    return InvoiceInversionsPreview_F.make_preview_pdf(formatted_data)
