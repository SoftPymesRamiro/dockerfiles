...
class InventoryArching(DocumentHeader):
    """
    ...
    def get_by_id(id):
        ...
        """
        inventory_arching = session.query(DocumentHeader).get(id)
        return inventory_arching

    @staticmethod
    def save_inventory_arching(data, short_word):
        """
        ...
        try:
            ...
            for dd in document_details:
                ...
                dd.save()
            InventoryArching.save_accounting(document_header, document_details)
            session.commit()
            inventory_arching = session.query(DocumentHeader).get(document_header_id)
            inventory_arching = DocumentHeader.export_data(inventory_arching)
            return document_header_id, inventory_arching['documentNumber']
    ...
    @staticmethod
    def update_inventory_arching(id_inventory_arching, data):
        """
        Allow update a inventory arching according to data and its identifier
        :param id_inventory_arching: identifier by inventory arching to update
        ...
        try:
            ...
            if id_inventory_arching != data['documentHeaderId']:
                abort(400)
            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_inventory_arching)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_inventory_arching)
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = InventoryArching.update_document_details(document_header, data, dds,
                                                                   document_detail_list,
                                                                   id_inventory_arching)
            # Genera la contabilidad
            InventoryArching.update_accounting(id_inventory_arching, document_header, all_document_details)
            session.commit()
    ...
    def delete_inventory_arching(id_inventory_arching):
        """
        Allow delete a inventory arching according its identifier
        :param id_inventory_arching: Id remission
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_inventory_arching).first()
            if document_header is None:
            ...
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == id_inventory_arching).all()
    ...
    @staticmethod
    def get_accounting_by_inventory_arching_id(inventory_arching_id):
        """
        Allow get accounting by sale item id
        :param inventory_arching_id: sale item id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == inventory_arching_id).all()
    ...
    def save_accounting(document_header, document_details):
        ...
        from ....models import InventoryArchingAccounting
        accounting = InventoryArchingAccounting(document_header)
    ...
    @staticmethod
    def update_accounting(id_inventory_arching, dh, all_document_details):
        """
        Allow update accounting records
        :param id_inventory_arching: id purchase item
        :param dh: document header object
        :param all_document_details: document details list
        :return: None
        """
        accounting_records = InventoryArching.get_accounting_by_inventory_arching_id(id_inventory_arching)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            InventoryArching.save_accounting(dh, all_document_details)
