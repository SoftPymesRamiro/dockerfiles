...
from ....models import DocumentHeader, DocumentType, DocumentDetail, Customer,\
    AccountingRecord, IVAType, Image, Serial, SerialDetail, Warehouse
...
class DistressedCost(DocumentHeader):
    """
    DistressedCost is an abstract document which is used to stand in for a
    ...
    def get_by_id(id):
        ...
        """
        distressed_cost = session.query(DocumentHeader).get(id)
        return distressed_cost

    @staticmethod
    def save_distressed_cost(data, short_word):
        """
        ...
        try:
            ...
            for dd in document_details:
                ...
                dd.save()
            # DistressedCost.save_accounting(document_header, document_details)
            session.commit()
            distressed_cost = session.query(DocumentHeader).get(document_header_id)
            distressed_cost = DocumentHeader.export_data(distressed_cost)
            return document_header_id, distressed_cost['documentNumber']
    ...
    @staticmethod
    def update_distressed_cost(id_distressed_cost, data):
        """
        Allow update a distressed cost document according to data and its identifier
        :param id_distressed_cost: identifier by distressed cost to update
        :param data: information by new document
        :exception: An error occurs when update not performance
        :return: document update in JSON format
        """
        try:
            # Valida que el id enviado en la url sea el mismo enviado en el data
            if id_distressed_cost != data['documentHeaderId']:
                abort(400)
            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_distressed_cost)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_distressed_cost)
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = DistressedCost.update_document_details(document_header, data, dds,
                                                                          document_detail_list,
                                                                          id_distressed_cost)
            # Genera la contabilidad
            # DistressedCost.update_accounting(id_distressed_cost, document_header, all_document_details)
            session.commit()
    ...
    def delete_distressed_cost(id_distressed_cost):
        """
        Allow delete a distressed cost document according its identifier
        :param id_distressed_cost: Id document
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_distressed_cost).first()
            if document_header is None:
            ...
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == id_distressed_cost).all()
    ...
    @staticmethod
    def get_accounting_by_distressed_cost_id(distressed_cost_id):
        """
        Allow get accounting by document id
        :param distressed_cost_id: document id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == distressed_cost_id).all()
    ...
    def save_accounting(document_header, document_details):
        ...
        from ....models import DistressedCostAccounting
        accounting = DistressedCostAccounting(document_header)
    ...
    @staticmethod
    def update_accounting(id_distressed_cost, dh, all_document_details):
        """
        Allow update accounting records
        :param id_distressed_cost: id purchase item
        :param dh: document header object
        :param all_document_details: document details list
        :return: None
        """
        accounting_records = DistressedCost.get_accounting_by_distressed_cost_id(id_distressed_cost)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            DistressedCost.save_accounting(dh, all_document_details)
