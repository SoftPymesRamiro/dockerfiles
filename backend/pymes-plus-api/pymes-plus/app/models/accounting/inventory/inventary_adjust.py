...
from ....reports import InventaryAdjustPreview
# from ....reports import InventaryAdjustPreviewM
# from ....reports import InventaryAdjustPreviewF
from ....utils.converters import format_cc_or_nit

class InventaryAdjust(DocumentHeader):
    """
    InventaryAdjust is a abstract document which is used to stand in for a
    ...
    def get_by_id(id):
        ...
        """
        inventary_adjust = session.query(DocumentHeader).get(id)
        return inventary_adjust

    @staticmethod
    def save_inventary_adjust(data, short_word):
        """
        ...
        try:
            ...
            InventaryAdjust.save_document_details(document_header, document_header_id, document_details)
            InventaryAdjust.save_accounting(document_header, document_details)
            session.commit()
            inventary_adjust = session.query(DocumentHeader).get(document_header_id)
            inventary_adjust = DocumentHeader.export_data(inventary_adjust)
            return document_header_id, inventary_adjust['documentNumber']
    ...
    @staticmethod
    def update_inventary_adjust(id_inventary_adjust, data):
        """
        Allow updater a sale item according to data and its identifier
        :param id_inventary_adjust: identifier by sale remission to update
        ...
        try:
            ...
            if id_inventary_adjust != data['documentHeaderId']:
                abort(400)
            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_inventary_adjust)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_inventary_adjust)
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = InventaryAdjust.update_document_details(document_header, data, dds,
                                                                   document_detail_list,
                                                                   id_inventary_adjust)
            # Genera la contabilidad
            InventaryAdjust.update_accounting(id_inventary_adjust, document_header, all_document_details)
            session.commit()
    ...
    def delete_inventary_adjust(id_inventary_adjust):
        ...
        :param id_inventary_adjust: Id remission
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_inventary_adjust).first()
            if document_header is None:
            ...
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == id_inventary_adjust).all()
    ...
    @staticmethod
    def get_accounting_by_inventary_adjust_id(inventary_adjust_id):
        """
        Allow get accounting by sale item id
        :param inventary_adjust_id: sale item id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == inventary_adjust_id).all()
        ...
        for dd in document_details:
            ...
            if dd.listSerials is not None:
                for s in dd.listSerials:
                    InventaryAdjust.save_serials(s, dd)
    ...
    def save_accounting(document_header, document_details):
        ...
        from ....models import InventaryAdjustAccounting
        accounting = InventaryAdjustAccounting(document_header)
    ...
    @staticmethod
    def update_accounting(id_inventary_adjust, dh, all_document_details):
        """
        Allow update accounting records
        :param id_inventary_adjust: id purchase item
        :param dh: document header object
        :param all_document_details: document details list
        :return: None
        """
        accounting_records = InventaryAdjust.get_accounting_by_inventary_adjust_id(id_inventary_adjust)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            InventaryAdjust.save_accounting(dh, all_document_details)
        ...
        try:
            if document_type == 'D':
                ...
                formatted_data = {
                    ...
                    'dependency': '' if not preview_data.dependency
                    else preview_data.dependency.name,


                }

                if format_type == 'P':
                    return InventaryAdjustPreview.make_preview_pdf(formatted_data)
