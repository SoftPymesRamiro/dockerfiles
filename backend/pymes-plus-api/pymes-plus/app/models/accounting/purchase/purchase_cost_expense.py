# -*- coding: utf-8 -*-
#########################################################
# Purchase Cost And Expenses Module
#########################################################
...
from ....models import DocumentHeader, DocumentType, DocumentDetail,PaymentDetail,\
    Serial, SerialDetail, PaymentReceipt, Provider, PUC, Branch, Division, Section, Dependency, \
...
from ....reports import InvoicePurchaseCostandExpensesPreview
from ....reports import InvoicePurchaseCostandExpensesPreviewM
from datetime import timedelta
from math import ceil

class PurchaseCostAndExpenses(DocumentHeader):
    """InvoicePurchaseCostAndExpenses as a public model class.
    note::
    """
    @staticmethod
    def export_data(data):
        ...
        return {
            ...
            'withholdingTaxPUC': None if data.withholdingTaxPUC is None else data.withholdingTaxPUC.export_data(),
    ...
    @staticmethod
    def save_purchase_cost_expense(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            document_details = PurchaseCostAndExpenses.import_details(document_detail_list)
            ...
            PurchaseCostAndExpenses.save_document_details(document_header, document_header_id, document_details)
            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            PurchaseCostAndExpenses.save_exchange_rate(document_header)
            payment_recipt = None
            if 'paymentReceipt' in data and data['paymentReceipt']:
                ...
                payment_recipt = PurchaseCostAndExpenses.import_payment_receipt(payment_recipt)
                # Guarda el payment Receipt con los payment details
                PurchaseCostAndExpenses.save_payment_receipt(document_header, payment_recipt)
            # Proceso de contabilizacion
            PurchaseCostAndExpenses.save_accounting(document_header, document_details, payment_recipt)
            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            PurchaseCostAndExpenses.validate_state_document(document_header, data, document_details)
            # session.expire_all()
    ...
    @staticmethod
    def update_purchase_cost_expense(id_purchase_cost_expense, data):
        """
        Allow updater a purchase item according to data and its identifier
        :param id_purchase_cost_expense: identifier by purchase order to update
        ...
        try:
            ...
            if id_purchase_cost_expense != data['documentHeaderId']:
                abort(400)
            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_purchase_cost_expense)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_purchase_cost_expense)
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = PurchaseCostAndExpenses.update_document_details(document_header, data, dds, document_detail_list,
                                                                             id_purchase_cost_expense)
            # Consulta el payment Receipt
            receipt_saved = PaymentReceipt.get_receipt_by_document_identifier(id_purchase_cost_expense)
            payment_receipt = data['paymentReceipt']
            if len(payment_receipt) > 0:
                payment_receipt = PurchaseCostAndExpenses.update_payment_receipt(document_header, data, receipt_saved,
                                                                          payment_receipt, id_purchase_cost_expense)
            else:
                payment_receipt_saved = session.query(PaymentReceipt.paymentReceiptId) \
                    .filter(PaymentReceipt.documentHeaderId == id_purchase_cost_expense)
            ...
            PurchaseCostAndExpenses.update_accounting(
                id_purchase_cost_expense, document_header, all_document_details, payment_receipt)
            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            PurchaseCostAndExpenses.validate_state_document(document_header, data, all_document_details)
    ...
    @staticmethod
    def get_expenses_accounts(puc_id=None, **kwargs):
        """
        Allow obtain all accounts expenses according to pucid
        :param puc_id: puc identifier
        ...
        puc_exp = session.query(PUC) \
            .filter(PUC.pucId == puc_id)\
            .first()

        if puc_exp and puc_exp.deferredInterest and puc_exp.deferredIncome:
            return puc_exp

        branch = Branch.get_branch_by_id(branch_id)
        # Obtiene los valores por defecto por branch id
        default_value = DefaultValue.get_default_value_by_branch_id(branch_id)
        level = default_value.branch.company.expenseLevel

        puc = {
            'D': Division.get_division_by_id(division_id).puc if division_id else None,  # División
            'S': Section.get_section_by_id(section_id).puc if section_id else None,  # Sección
            'P': Dependency.get_dependency_by_id(dependency_id).puc if dependency_id else None,  # Dependencia
        }.get(level)

        if puc:
            list_puc = PUC.get_list_puc(branch.companyId)
            pu = [p for p in list_puc if p.costCenter and
                  p.pucClass + p.pucSubClass == puc_exp.pucClass + puc_exp.pucSubClass]
            if pu and len(pu) > 0:
                oPUC = [p for p in list_puc if
                ...
                if oPUC and len(oPUC) > 0:
                    return PUC.export_account_data(oPUC[0])
                else:
                    raise ValueError("")
            else:
                return None
    ...
    @staticmethod
    def get_associate_accounts(puc_id=None, **kwargs):
        """
        Allow obtain all accounts associates according to puc_id
        :param puc_id: puc identifier
        :param kwargs: others params
        :return: list with all associates
        """
        page_size = kwargs.get("page_size")
        page_number = kwargs.get("page_number")
        search = kwargs.get("search").upper()
        words = kwargs.get("words")

        if page_size and page_number:
            associate_accounts = session.query(PUC) \
                .filter(PUC.pucId == puc_id) \
                .first()
            list_puc = [PUC.export_account_data(puc)
                    for puc in associate_accounts.pucs
                        [(int(page_number) - 1) * int(page_size):
                               (int(page_size) +  ((int(page_number) - 1) * int(page_size))
                                if page_size is not None else None)] ]

            total_count = len(associate_accounts.pucs)
            total_pages = int(ceil(total_count / float(page_size)))
            return {'listPUC': list_puc, 'totalCount': total_count, 'totalPages': total_pages}
        else:
            associate_accounts = session.query(PUC) \
                .filter(PUC.pucId == puc_id) \
                .first()
            ...
            list_puc = [PUC.export_account_data(puc)
                        for puc in associate_accounts.pucs
                        if search in puc.name or
                        search in (puc.pucClass + puc.pucSubClass + puc.account + puc.subAccount + puc.auxiliary1)
                        ]

            return {'data':list_puc}
    ...
    def get_by_id(id_purchase):
        ...
        """
        purchase_cost_expense = session.query(DocumentHeader).filter(
            DocumentHeader.documentHeaderId == id_purchase
        ).first()

        return purchase_cost_expense

    @staticmethod
    def update_payment_receipt(document_header, data, payment_receipt_old, payment_receipt_new, id_purchase_cost_expense):
        """
        ...
        :param id_purchase_cost_expense: id advance third
    ...
    @staticmethod
    def delete_purchase_cost_expense(id_purchase_cost_expense):
        """
        Allow delete a purchase item according its identifier
        :param id_purchase_cost_expense: Id item
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_purchase_cost_expense).first()
            if document_header is None:
                response = jsonify({'code': 404, 'message': 'Purchase Item Not Found'})
            ...
            accounting_records = session.query(AccountingRecord)\
                .filter(or_(AccountingRecord.documentHeaderId == id_purchase_cost_expense,
                            AccountingRecord.crossDocumentHeaderId == id_purchase_cost_expense)).delete()
            session.flush()
    ...
    @staticmethod
    def get_accounting_by_purchase_cost_expense_id(purchase_cost_expense_id):
        """
        Allow get accounting by purchase itemn id
        :param purchase_cost_expense_id: purchase item id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == purchase_cost_expense_id).all()
    ...
    def delete_serials(document_detail_id):
        ...
        :param id_purchase_cost_expense: id purchase item
        :return: None
        """
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        # Proceso de contabilizacion
        from ....models import PurchaseCostAndExpensesAccounting
        accounting = PurchaseCostAndExpensesAccounting(document_header)
    ...
    @staticmethod
    def update_accounting(id_purchase_cost_expense, dh, all_document_details,payment_receipt):
        """
        ...
        :return: None
        """
        accounting_records = PurchaseCostAndExpenses.get_accounting_by_purchase_cost_expense_id(id_purchase_cost_expense)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            PurchaseCostAndExpenses.save_accounting(dh, all_document_details,payment_receipt)
        ...
        for dd in document_details:
            ...
            if dd.listSerials is not None:
                for s in dd.listSerials:
                    PurchaseCostAndExpenses.save_serial(s, dd)
    ...
    @staticmethod
    def update_document_details(dh, data, dds, document_detail_list, id_purchase_cost_expense):
        """
        ...
        for d in document_detail_list:
            if 'documentDetailId' not in d:
                ...
                new_detail.documentHeaderId = id_purchase_cost_expense
        ...
        for dd in all_document_details:
            ...
            if dd.listSerials is not None and (not dd.sourceDocumentDetail):
                PurchaseCostAndExpenses.delete_serials(dd.documentDetailId)  # elimina los seriales
                for s in dd.listSerials:
                    PurchaseCostAndExpenses.save_serial(s, dd) #crea nuevamente los seriales
    ...
    def get_document_preview(document_header_id, format_type='P', document_type='D'):
        ...
        try:
            if document_type == 'D':
                ...
                if format_type == 'P':
                    return InvoicePurchaseCostandExpensesPreview.make_preview_pdf(formatted_data)
                elif format_type == 'M':
                    return InvoicePurchaseCostandExpensesPreviewM.make_preview_pdf(formatted_data)
