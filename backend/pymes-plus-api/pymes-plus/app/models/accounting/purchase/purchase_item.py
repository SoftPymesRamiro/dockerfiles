...
from ....reports import InvoicePurchaseItemPreview
from ....reports import InvoicePurchaseItemPreview_M
from datetime import timedelta
from ....utils.converters import format_cc_or_nit

class PurchaseItem(DocumentHeader):
    """InvoicePurchaseItem as a public model class.
    note::
    """

    @staticmethod
    def export_purchase_item(data):
        """
    ...
    @staticmethod
    def save_purchase_item(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            document_details = PurchaseItem.import_details(document_detail_list)

            # Obtiene el documenttypeid ya que solo esta recibiendo el shortword
            document_type_id = session.query(DocumentType.documentTypeId).filter(
                DocumentType.shortWord == short_word).scalar()

            source__docId = session.query(DocumentType.documentTypeId).filter(
            ...
            document_header.sourceDocumentTypeId = source__docId
            ...
            PurchaseItem.save_document_details(document_header, document_header_id, document_details)

            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            PurchaseItem.save_exchange_rate(document_header)
            ...
            if 'paymentReceipt' in data and data['paymentReceipt']:
                ...
                payment_recipt = PurchaseItem.import_payment_receipt(payment_recipt)
                # Guarda el payment Receipt con los payment details
                PurchaseItem.save_payment_receipt(document_header, payment_recipt)

            # Proceso que contabiliza
            PurchaseItem.save_accounting(document_header, document_details, payment_recipt)

            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            PurchaseItem.validate_state_document(document_header, data, document_details)
            # session.expire_all()
            purchase_item = session.query(DocumentHeader).get(document_header_id)
            purchase_item = DocumentHeader.export_data(purchase_item)

            return document_header_id, purchase_item['documentNumber']
    ...
    @staticmethod
    def update_purchase_item(id_purchase_item, data):
        """
        ...
        try:
            ...
            # Actualiza detalles
            all_document_details = PurchaseItem.update_document_details(document_header, data, dds, document_detail_list,
                                                                             id_purchase_item)
            # Consulta el payment Receipt
            receipt_saved = PaymentReceipt.get_receipt_by_document_identifier(id_purchase_item)

            payment_receipt = data['paymentReceipt']
            if len(payment_receipt) > 0:
                payment_receipt = PurchaseItem.update_payment_receipt(document_header, data, receipt_saved,
                                                                          payment_receipt, id_purchase_item)
            else:
                payment_receipt_saved = session.query(PaymentReceipt.paymentReceiptId) \
                    .filter(PaymentReceipt.documentHeaderId == id_purchase_item)
            ...
            PurchaseItem.update_accounting(id_purchase_item, document_header, all_document_details, payment_receipt)

            session.commit()

            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            PurchaseItem.validate_state_document(document_header, data, all_document_details)
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        from ....models import PurchaseItemAccounting
        accounting = PurchaseItemAccounting(document_header)
    ...
    def update_accounting(id_purchase_item, dh, all_document_details,payment_receipt):
        ...
        :return: None
        """
        accounting_records = PurchaseItem.get_accounting_by_purchase_item_id(id_purchase_item)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            PurchaseItem.save_accounting(dh, all_document_details,payment_receipt)
    ...
    def update_document_details(dh, data, dds, document_detail_list, id_purchase_item):
        ...
        for dd in all_document_details:
            ...
            if dd.listSerials is not None and (not dd.sourceDocumentDetail):
                PurchaseItem.delete_serials(dd.documentDetailId)  # elimina los seriales
                for s in dd.listSerials:
                    PurchaseItem.save_serial(s, dd) #crea nuevamente los seriales
        ...
        try:
            if document_type == 'D':
                ...
                formatted_data = {
                    ...
                    'provider_cellPhone': ' ' if not preview_data.provider.cellPhone
                    else preview_data.provider.cellPhone,
                ...
                if format_type == 'P':
                    return InvoicePurchaseItemPreview.make_preview_pdf(formatted_data)
                elif format_type == 'M':
                    return InvoicePurchaseItemPreview_M.make_preview_pdf(formatted_data)
