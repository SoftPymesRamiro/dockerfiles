...
from ....models import DocumentHeader, DocumentType, DocumentDetail,PaymentDetail, PaymentReceipt, Provider,\
...
from ....reports import InvoicePurchaseDeferredPreview
from ....reports import InvoicePurchaseDeferredPreviewM
from datetime import timedelta

class PurchaseDeferred(DocumentHeader):
    """InvoicePurchaseDeferred as a public model class.
    note::
    """
    ...
    def get_by_id(id_purchase):
        ...
        purchase_deferred = session.query(DocumentHeader).filter(
            DocumentHeader.documentHeaderId == id_purchase
        ).first()

        return PurchaseDeferred.export_data(purchase_deferred)

    @staticmethod
    def save_purchase_deferred(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            document_details = PurchaseDeferred.import_details(document_detail_list)
            ...
            PurchaseDeferred.save_document_details(document_header, document_header_id, document_details)
            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            PurchaseDeferred.save_exchange_rate(document_header)
            payment_recipt = None
            if 'paymentReceipt' in data and data['paymentReceipt']:
                ...
                payment_recipt = PurchaseDeferred.import_payment_receipt(payment_recipt)
                # Guarda el payment Receipt con los payment details
                PurchaseDeferred.save_payment_receipt(document_header, payment_recipt)
            # Proceso de contabilizacion
            PurchaseDeferred.save_accounting(document_header, document_details, payment_recipt)
            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            PurchaseDeferred.validate_state_document(document_header, data, document_details)
            # session.expire_all()
    ...
    @staticmethod
    def update_purchase_deferred(id_purchase_deferred, data):
        """
        Allow updater a purchase item according to data and its identifier
        :param id_purchase_deferred: identifier by purchase order to update
        ...
        try:
            ...
            if id_purchase_deferred != data['documentHeaderId']:
                abort(400)
            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_purchase_deferred)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_purchase_deferred)
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = PurchaseDeferred.update_document_details(document_header, data, dds, document_detail_list,
                                                                             id_purchase_deferred)
            # Consulta el payment Receipt
            receipt_saved = PaymentReceipt.get_receipt_by_document_identifier(id_purchase_deferred)
            payment_receipt = data['paymentReceipt']
            if len(payment_receipt) > 0:
                payment_receipt = PurchaseDeferred.update_payment_receipt(document_header, data, receipt_saved,
                                                                          payment_receipt, id_purchase_deferred)
            else:
                payment_receipt_saved = session.query(PaymentReceipt.paymentReceiptId) \
                    .filter(PaymentReceipt.documentHeaderId == id_purchase_deferred)
            ...
            PurchaseDeferred.update_accounting(
                id_purchase_deferred, document_header, all_document_details, payment_receipt)
            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            PurchaseDeferred.validate_state_document(document_header, data, all_document_details)
    ...
    @staticmethod
    def update_payment_receipt(document_header, data, payment_receipt_old, payment_receipt_new, id_purchase_deferred):
        """
        ...
        :param id_purchase_deferred: id advance third
    ...
    def delete_purchase_deferred(id_purchase_deferred):
        ...
        :param id_purchase_deferred: Id item
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_purchase_deferred).first()
            if document_header is None:
            ...
            accounting_records = session.query(AccountingRecord)\
                .filter(or_(AccountingRecord.documentHeaderId == id_purchase_deferred,
                            AccountingRecord.crossDocumentHeaderId == id_purchase_deferred)).delete()
            session.flush()
    ...
    @staticmethod
    def get_accounting_by_purchase_deferred_id(purchase_deferred_id):
        """
        Allow get accounting by purchase itemn id
        :param purchase_deferred_id: purchase item id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == purchase_deferred_id).all()
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        # Proceso de contabilizacion
        from ....models import PurchaseDeferredAccounting
        accounting = PurchaseDeferredAccounting(document_header)
    ...
    @staticmethod
    def update_accounting(id_purchase_deferred, dh, all_document_details,payment_receipt):
        """
        Allow update accounting records
        :param id_purchase_deferred: id purchase item
        :param dh: document header object
        :param all_document_details: document details list
        :return: None
        """
        accounting_records = PurchaseDeferred.get_accounting_by_purchase_deferred_id(id_purchase_deferred)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            PurchaseDeferred.save_accounting(dh, all_document_details,payment_receipt)
    ...
    @staticmethod
    def update_document_details(dh, data, dds, document_detail_list, id_purchase_deferred):
        """
        ...
        for d in document_detail_list:
            if 'documentDetailId' not in d:
                ...
                new_detail.documentHeaderId = id_purchase_deferred
        ...
        try:
            if document_type == 'D':
                ...
                formatted_data = {
                    ...
                    'sourceControlPrefix': preview_data.sourceDocumentHeader,
                ...
                if format_type == 'P':
                    return InvoicePurchaseDeferredPreview.make_preview_pdf(formatted_data)
                elif format_type == 'M':
                    return InvoicePurchaseDeferredPreviewM.make_preview_pdf(formatted_data)
