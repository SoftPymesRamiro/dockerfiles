...
from ....reports import InvoiceProviderNotesPreview
from ....reports import InvoiceProviderNotesPreviewM
from datetime import timedelta

class ProviderNote(DocumentHeader):
    """InvoiceProviderNote as a public model class.
    note::
    """
    ...
    @staticmethod
    def save_provider_note(data, short_word, source_short_word):
        """
        ...
        try:
            ...
            document_details = ProviderNote.import_details(document_detail_list)
            ...
            ProviderNote.save_document_details(document_header, document_header_id, document_details)
            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            ProviderNote.save_exchange_rate(document_header)
            # Proceso que contabiliza
            ProviderNote.save_accounting(document_header, document_details, None)
            # Persistencia a base de datos
            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            ProviderNote.validate_state_document(document_header, data, document_details)
            # session.expire_all()
    ...
    @staticmethod
    def update_provider_note(id_provider_note, data):
        """
        Allow updater a purchase item according to data and its identifier
        :param id_provider_note: identifier by purchase order to update
        ...
        try:
            ...
            if id_provider_note != data['documentHeaderId']:
                abort(400)
            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_provider_note)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_provider_note)
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = ProviderNote.update_document_details(document_header, data, dds, document_detail_list,
                                                                        id_provider_note)
            # Genera la contabilidad
            ProviderNote.update_accounting(id_provider_note, document_header, all_document_details, None)
            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            ProviderNote.validate_state_document(document_header, data, all_document_details)
    ...
    @staticmethod
    def delete_provider_note(id_provider_note):
        """
        Allow delete a purchase item according its identifier
        :param id_provider_note: Id item
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_provider_note).first()
            if document_header is None:
                response = jsonify({'code': 404, 'message': 'Provider note Not Found'})
            ...
            accounting_records = session.query(AccountingRecord) \
                .filter(or_(AccountingRecord.documentHeaderId == id_provider_note,
                            AccountingRecord.crossDocumentHeaderId == id_provider_note)).delete()
            session.flush()
    ...
    @staticmethod
    def get_accounting_by_provider_note_id(provider_note_id):
        """
        Allow get accounting by purchase itemn id
        :param provider_note_id: purchase item id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == provider_note_id).all()
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        ret_value = []
        if document_header.documentType.shortWord == "CP":
            from ....models import ProviderNoteAccountingCP
            accounting = ProviderNoteAccountingCP(document_header)
        ...
        elif document_header.documentType.shortWord == "DP":
            from ....models import ProviderNoteAccountingDP
            accounting = ProviderNoteAccountingDP(document_header)
            # Retorna lista con los resgistros de contabilizacion
            ret_value = accounting.do_account(document_header, document_details, payment_receipt)
    ...
    def update_accounting(id_provider_note, dh, all_document_details, payment_receipt):
        ...
        :return: None
        """
        accounting_records = ProviderNote.get_accounting_by_provider_note_id(id_provider_note)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            ProviderNote.save_accounting(dh, all_document_details, payment_receipt)
    ...
    @staticmethod
    def update_document_details(dh, data, dds, document_detail_list, id_provider_note):
        """
        ...
        for d in document_detail_list:
            if 'documentDetailId' not in d:
                ...
                new_detail.documentHeaderId = id_provider_note
        ...
        for dd in all_document_details:
            ...
            if dd.listSerials is not None and (not dd.sourceDocumentDetail):
                ProviderNote.delete_serials(dd.documentDetailId)  # elimina los seriales
                for s in dd.listSerials:
                    ProviderNote.save_serial(s, dd)  # crea nuevamente los seriales
    ...
    def get_document_preview(document_header_id, format_type='P', document_type='D'):
        ...
        try:
            if document_type == 'D':
                ...
                if format_type == 'P':
                    return InvoiceProviderNotesPreview.make_preview_pdf(formatted_data)
                elif format_type == 'M':
                    return InvoiceProviderNotesPreviewM.make_preview_pdf(formatted_data)
            elif document_type == 'V':
                # Comprobante de egreso
                # return AdvanceThird.get_advance_third_preview(document_header_id, format_type) #original
