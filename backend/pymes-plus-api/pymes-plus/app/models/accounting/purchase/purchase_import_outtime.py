# -*- coding: utf-8 -*-
#########################################################
# Purchase Import Out Time Module
#########################################################
import copy
...
from ....models import DocumentHeader, DocumentType, DocumentDetail,PaymentDetail,\
    ...
    ExchangeRate, DefaultValue, AccountingRecord, PaymentMethod, IVAType, Image, AdvanceThird, Import
from flask import jsonify, abort, g
from datetime import datetime
from sqlalchemy import or_, not_, and_, func
from sqlalchemy.orm import aliased, Load, joinedload, make_transient
from ....exceptions import ValidationError, InternalServerError
from ...referential.general_parameter import GeneralParameter
from ....reports import InvoicePurchaseImportOutTimePreview
from datetime import timedelta


class PurchaseImportOutTime(DocumentHeader):
    """PurchaseImportOutTime as a public model class.
    note::
    """
    ...
    def get_by_id(id_purchase):
        ...
        purchase_import_outtime = session.query(DocumentHeader).filter(
            DocumentHeader.documentHeaderId == id_purchase
        ).first()

        return purchase_import_outtime

    @staticmethod
    def validate_document_is_out_time(document_header):
        """
        Allow validate an invoice import and import out time, because they're using the same short word
        :param document_header: document header object 
        :return: True or False
        """
        try:
            doch = session.query(DocumentHeader.closingType, DocumentType.shortWord, DocumentHeader.documentNumber)\
            ...
            # Valida que la importacion involucrada tenga varios documentos y ademas que exista algun cierre final
            if len(doch) > 1 and any(a for a, b, c in doch):
                # Extrae el ultimo registro, si es FM es extemporanea, de lo contrario es FM normal
                if doch[-1][1] == 'FM' and doch[-1][2] == document_header.documentNumber:
                    return True
            return False
    ...
    @staticmethod
    def save_purchase_import_outtime(data, short_word):
        """
        ...
        try:
            ...
            document_details = PurchaseImportOutTime.import_details(document_detail_list)
            ...
            #     if not data['sourceDocumentOrigin'] == "FP" else None
            ...
            PurchaseImportOutTime.save_document_details(document_header, document_header_id, document_details)

            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            PurchaseImportOutTime.save_exchange_rate(document_header)
            ...
            if 'paymentReceipt' in data and data['paymentReceipt']:
                ...
                payment_recipt = PurchaseImportOutTime.import_payment_receipt(payment_recipt)
                # Guarda el payment Receipt con los payment details
                PurchaseImportOutTime.save_payment_receipt(document_header, payment_recipt)

            # Proceso de contabilizacion
            PurchaseImportOutTime.save_accounting(document_header, document_details, payment_recipt)

            # Reemplazo de factura
            # if document_header.sourceDocumentHeaderId is not None:
            PurchaseImportOutTime.replace_invoice(document_header)

            session.commit()
            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            # PurchaseImportOutTime.validate_state_document(document_header, data, document_details)
            # session.expire_all()
    ...
    @staticmethod
    def update_payment_receipt(document_header, data, payment_receipt_old, payment_receipt_new, id_purchase_import_outtime):
        """
        ...
        :param id_purchase_import_outtime: id advance third
    ...
    @staticmethod
    def update_purchase_import_outtime(id_purchase_import_outtime, data):
        """
        Allow updater a purchase import according to data and its identifier
        :param id_purchase_import_outtime: identifier by purchase order to update
        ...
        try:
            ...
            if id_purchase_import_outtime != data['documentHeaderId']:
                abort(400)

            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_purchase_import_outtime)

            all_document_list = session.query(DocumentHeader) \
                .join(DocumentType, DocumentType.documentTypeId == DocumentHeader.documentTypeId) \
                .filter(DocumentType.shortWord == 'FM',
                        DocumentHeader.importId == document_header.importId,
                        DocumentHeader.branchId == document_header.branchId).all()

            all_document_list_copy = copy.deepcopy(all_document_list)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_purchase_import_outtime)

            document_detail_list = data['documentDetails']

            # Actualiza detalles
            all_document_details = PurchaseImportOutTime.update_document_details(
                document_header, data, dds, document_detail_list, id_purchase_import_outtime)
            # Consulta el payment Receipt
            receipt_saved = PaymentReceipt.get_receipt_by_document_identifier(id_purchase_import_outtime)

            payment_receipt = data['paymentReceipt']
            if len(payment_receipt) > 0:
                payment_receipt = PurchaseImportOutTime.update_payment_receipt(
                    document_header, data, receipt_saved, payment_receipt, id_purchase_import_outtime)
            else:
                payment_receipt_saved = session.query(PaymentReceipt.paymentReceiptId) \
                    .filter(PaymentReceipt.documentHeaderId == id_purchase_import_outtime)
            ...
            PurchaseImportOutTime.update_accounting(
                id_purchase_import_outtime, document_header, all_document_details, payment_receipt)

            PurchaseImportOutTime.replace_invoice(document_header, all_document_list_copy, update=True)

            session.commit()

            # Ejecuta procedimiento almacenado para cambiar el estado al documento origen
            # PurchaseImportOutTime.validate_state_document(document_header, data, all_document_details)
    ...
    @staticmethod
    def delete_purchase_import_outtime(id_purchase_import_outtime):
        """
        Allow delete a purchase item according its identifier
        :param id_purchase_import_outtime: Id item
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_purchase_import_outtime).first()
            if document_header is None:
                response = jsonify({'code': 404, 'message': 'Purchase Import out time Not Found'})
            ...
            accounting_records = session.query(AccountingRecord)\
                .filter(or_(AccountingRecord.documentHeaderId == id_purchase_import_outtime,
                            AccountingRecord.crossDocumentHeaderId == id_purchase_import_outtime)).delete()
            session.flush()
    ...
    @staticmethod
    def get_accounting_by_purchase_import_outtime_id(purchase_import_outtime_id):
        """
        Allow get accounting by purchase itemn id
        :param purchase_import_outtime_id: purchase item id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == purchase_import_outtime_id).all()
    ...
    def delete_serials(document_detail_id):
        ...
        :param id_purchase_import_outtime: id purchase item
        :return: None
        """
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        # Proceso de contabilizacion
        from ....models import PurchaseImportOutTimeAccounting
        accounting = PurchaseImportOutTimeAccounting(document_header)
    ...
    @staticmethod
    def update_accounting(id_purchase_import_outtime, dh, all_document_details,payment_receipt):
        """
        ...
        :return: None
        """
        accounting_records = PurchaseImportOutTime.get_accounting_by_purchase_import_outtime_id(id_purchase_import_outtime)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            PurchaseImportOutTime.save_accounting(dh, all_document_details,payment_receipt)
        ...
        for dd in document_details:
            ...
            if dd.listSerials is not None:
                for s in dd.listSerials:
                    PurchaseImportOutTime.save_serial(s, dd)
    ...
    @staticmethod
    def update_document_details(dh, data, dds, document_detail_list, id_purchase_import_outtime):
        """
        ...
        for d in document_detail_list:
            if 'documentDetailId' not in d:
                ...
                new_detail.documentHeaderId = id_purchase_import_outtime
        ...
        for dd in all_document_details:
            ...
            if dd.listSerials is not None and (not dd.sourceDocumentDetail):
                PurchaseImportOutTime.delete_serials(dd.documentDetailId)  # elimina los seriales
                for s in dd.listSerials:
                    PurchaseImportOutTime.save_serial(s, dd) #crea nuevamente los seriales
    ...
    @staticmethod
    def replace_invoice(document_header, all_document_list=None, update=False):
        """
        In progress
        :param document_header: 
        :return: 
        """
        dh_close_import_list = []
        dh_close_import_list_fm = []
        dh_close_import_list_an = []
        dh_close_import = None

        if all_document_list is None:
            all_document_list = session.query(DocumentHeader) \
                .join(DocumentType, DocumentType.documentTypeId == DocumentHeader.documentTypeId) \
                .filter(DocumentType.shortWord == 'FM',
                        DocumentHeader.importId == document_header.importId,
        ...
        # all_document_list = all_document_list.all()
        # # make_transient(all_document_list)
        dh_close_import_list_an = [a for a in all_document_list if a.annuled]
        dh_close_import_list_fm = [a for a in all_document_list if not a.annuled]

        # Consulta el documento padre
        dh_close_import_list = session.query(DocumentHeader) \
            .join(DocumentType, DocumentType.documentTypeId == DocumentHeader.documentTypeId) \
            .filter(DocumentType.shortWord == 'CM',
                    DocumentHeader.importId == document_header.importId,
                    DocumentHeader.branchId == document_header.branchId,
                    DocumentHeader.annuled == 0) \
            .all()

        dh_close_import = [a for a in dh_close_import_list if a.closingType][0]

        if len(dh_close_import_list_fm) > 0:
            search = dh_close_import_list_fm[0].documentType.shortWord
        else:
            search = None

        if document_header.sourceDocumentHeaderId is not None:
            # TODO: Actualizar importación
            # Anula la factura a reemplazar
            source_document_header = DocumentHeader.get_by_id(document_header.sourceDocumentHeaderId)
            cancel_invoice = source_document_header
            cancel_invoice.cancel()

            dh_close_import.importationValue -= source_document_header.subtotal
            dh_close_import.total -= source_document_header.subtotal
            #
            if not update:
                dh_close_import.total += document_header.subtotal
                dh_close_import.importationValue += document_header.subtotal
                dh_close_import.expenses = dh_close_import.importationValue - dh_close_import.subtotal
            #
            from ... import ClosePurchaseImport
            ClosePurchaseImport.update_close_purchase_import_external(dh_close_import)

        if not update and document_header.sourceDocumentHeaderId is None:
            dh_close_import.total += document_header.subtotal
            dh_close_import.importationValue += document_header.subtotal
            dh_close_import.expenses = dh_close_import.importationValue - dh_close_import.subtotal
            from ... import ClosePurchaseImport
        ...
        if update:
            ...
            result_dif2 = 0
            sub_annul = 0
            for ret in dh_close_import_list:
                if ret.documentType.shortWord == "CM":
                    importId = ret.importId
                    for retAn in dh_close_import_list_an:
                        if importId == retAn.importId:

                            # // subtotal cuando el documento esta anulado.
                            if retAn.annuled and retAn.documentNumber == document_header.documentNumber:

                                sub_annul = retAn.subtotal
                                if sub_annul > 0:
                    ...
                    for retfm in dh_close_import_list_fm:
                        if importId == retfm.importId:
                            if retfm.documentNumber == document_header.documentNumber:
                                resultDif = retfm.subtotal - document_header.subtotal
                                # // Cuando la vista (Gasto Extratemporaneo (FM)) trae un valor mayor a la base de datos
                                if resultDif < 0:
                                    resultDif2 = document_header.subtotal - retfm.subtotal
                                    dh_close_import.importationValue += resultDif2
                                    dh_close_import.total += resultDif2
                                    dh_close_import.expenses = dh_close_import.importationValue - dh_close_import.subtotal

                                # // Cuando la vista (Gasto Extratemporaneo (FM)) trae un valor menor
                                elif resultDif > 0:
                                    resultDif = retfm.subtotal - document_header.subtotal
                                    dh_close_import.importationValue -= resultDif
                                    dh_close_import.total -= resultDif
                                    dh_close_import.expenses = dh_close_import.importationValue - dh_close_import.subtotal

                                # // Cuando la vista (Gasto Extratemporaneo (FM)) trae el mismo valor
                                if resultDif == 0:
                                    dh_close_import.importationValue += document_header.subtotal
                                    dh_close_import.total += document_header.subtotal
                                    dh_close_import.importationValue -= document_header.subtotal
        ...
        if len(dh_close_import_list) > 1:
            import_1 = session.query(Import).get(document_header.importId)
            import_1.isOutTime = True
            session.add(import_1)
            session.flush()
    ...
    def get_document_preview(document_header_id, format_type='P', document_type='D'):
        ...
        try:
            if document_type == 'D':
                ...
                if format_type == 'P':
                    return InvoicePurchaseImportOutTimePreview.make_preview_pdf(formatted_data)
