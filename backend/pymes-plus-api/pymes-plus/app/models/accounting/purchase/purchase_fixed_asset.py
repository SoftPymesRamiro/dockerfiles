...
from ....models import DocumentHeader, DocumentType,PaymentDetail,\
    Provider, PaymentReceipt,Contract, DocumentDetail, Asset, \
...
from ....reports import InvoicePurchaseFixedAssetsPreview
from ....reports import InvoicePurchaseFixedAssetsPreviewM
from ....exceptions import ValidationError, InternalServerError
from ...referential.general_parameter import GeneralParameter
from datetime import timedelta

class PurchaseFixedAsset(DocumentHeader):
    ...
    def get_all_assets(branch_id):
        ...
        fixed_asset = session.query(Asset) \
            .join(AccountingRecord,
                  and_(AccountingRecord.assetId == Asset.assetId), isouter=True) \
    ...
    def get_by_id(id_purchase):
        ...
        """
        fixed_asset = session.query(DocumentHeader).filter(
    ...
    @staticmethod
    def save_purchase_fixed_asset(data, short_word, source_short_word=None):
        """
        ...
        try:
            ...
            document_details = PurchaseFixedAsset.import_details(document_detail_list)
            ...
            PurchaseFixedAsset.save_document_details(document_header, document_header_id, document_details)

            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            PurchaseFixedAsset.save_exchange_rate(document_header)
            ...
            if 'paymentReceipt' in data and data['paymentReceipt']:
                ...
                payment_recipt = PurchaseFixedAsset.import_payment_receipt(payment_recipt)
                # Guarda el payment Receipt con los payment details
                PurchaseFixedAsset.save_payment_receipt(document_header, payment_recipt)

            # Proceso de contabilizacion
            PurchaseFixedAsset.save_accounting(document_header, document_details, payment_recipt)
    ...
    @staticmethod
    def update_purchase_fixed_asset(id_purchase_asset, data):
        """
         Allow updater a advance thirds according to data and its identifier
         :param id_purchase_asset: identifier by purchase order to update
        ...
        try:
            ...
            if id_purchase_asset != data['documentHeaderId']:
                abort(400)

            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_purchase_asset)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_purchase_asset)

            document_detail_list = data['documentDetails']

            # Actualiza detalles
            all_document_details = PurchaseFixedAsset.update_document_details\
                (document_header, data, dds, document_detail_list,id_purchase_asset)

            # Consulta el payment Receipt
            receipt_saved = PaymentReceipt.get_receipt_by_document_identifier(id_purchase_asset)

            payment_receipt = data['paymentReceipt']
            if len(payment_receipt) > 0:
                payment_receipt = PurchaseFixedAsset.update_payment_receipt(document_header, data, receipt_saved,
                                                                          payment_receipt, id_purchase_asset)
            else:
                payment_receipt_saved = session.query(PaymentReceipt.paymentReceiptId) \
                    .filter(PaymentReceipt.documentHeaderId == id_purchase_asset)
            ...
            PurchaseFixedAsset.update_accounting(id_purchase_asset, document_header,
    ...
    @staticmethod
    def delete_purchase_fixed_asset(id_purchase_fixed_asset):
        """
        Allow delete a invoice contract according its identifier
        :param id_purchase_fixed_asset: Id item
        ...
        document_header = session.query(DocumentHeader).get(id_purchase_fixed_asset)
        if document_header is None:
            response = jsonify({'code': 404, 'message': 'purchase fixed asset Not Found'})
        ...
        try:
            ...
            receipt_ids = session.query(PaymentReceipt.paymentReceiptId) \
                .filter(PaymentReceipt.documentHeaderId == id_purchase_fixed_asset).subquery()
            ...
            receipt_deleted = session.query(PaymentReceipt) \
                .filter(PaymentReceipt.documentHeaderId == id_purchase_fixed_asset).delete()
            # Eliminar la contabilidad
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == id_purchase_fixed_asset).delete()
            session.flush()
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        # Proceso de contabilizacion
        from ....models import PurchaseFixedAssetAccounting
        accounting = PurchaseFixedAssetAccounting(document_header)
    ...
    @staticmethod
    def update_document_details(dh, data, dds, document_detail_list, id_purchase_fixed):
        """
        ...
        :param id_purchase_fixed: id purchase item
        ...
        for dd in delete_details:
            # restaura el saldo del item a eliminar si este tine padre
        ...
        for d in document_detail_list:
            if 'documentDetailId' not in d:
                ...
                new_detail.documentHeaderId = id_purchase_fixed
    ...
    def update_payment_receipt(document_header, data, payment_receipt_old,
                               payment_receipt_new, id_purchase_fixed_asset):
        """
        ...
        :param id_purchase_fixed_asset: id advance third
    ...
    @staticmethod
    def get_accounting_byid(id_purchase_fixed_asset):
        """
        ...
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == id_purchase_fixed_asset).all()
    ...
    @staticmethod
    def validate_state_of_document(document_header, data):
        """
        ...
        :return: None
        """
        if document_header.contractId is not None:
            source_contract = Contract.get_by_id(document_header.contractId)
            source_contract.state = 0
            source_contract.update()

    @staticmethod
    def get_accounting_by_purchase_asset(id_purchase_asset):
        """
        Allow get accounting by purchase itemn id
        :param id_purchase_asset: purchase item id
        :return: accounting record object list
        """
        try:
            accounting_records = session.query(AccountingRecord) \
                .filter(AccountingRecord.documentHeaderId == id_purchase_asset).all()
            return accounting_records
        except Exception as e:
            session.rollback()
            raise InternalServerError(e)
    ...
    @staticmethod
    def update_accounting(id_purchase_asset, dh, all_document_details, payment_receipt):
        """
        Allow update accounting records
        :param id_purchase_asset: id purchase item
        :param dh: document header object
        :param all_document_details: document details list
        :return: None
        """
        accounting_records = PurchaseFixedAsset.get_accounting_by_purchase_asset(id_purchase_asset)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            PurchaseFixedAsset.save_accounting(dh, all_document_details,payment_receipt)
    ...
    @staticmethod
    def get_invoicecontract_preview(id_purchase_fixed_asset, format_type='P'):
        """
        Allow obtain preview
        :param id_purchase_fixed_asset: identifier by advance third
        :param format_type: identifier by format
        :return:
        """
        formatted_data = {'info': 'info', 'checkBook': []}
    ...
    def get_document_preview(document_header_id, format_type='P', document_type='D'):
        ...
        try:
            if document_type == 'D':
                ...
                if format_type == 'P':
                    return InvoicePurchaseFixedAssetsPreview.make_preview_pdf(formatted_data)
                if format_type == 'M':
                    return InvoicePurchaseFixedAssetsPreviewM.make_preview_pdf(formatted_data)
