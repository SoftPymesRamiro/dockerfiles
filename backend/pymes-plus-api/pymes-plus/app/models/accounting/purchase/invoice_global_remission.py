...
from ....reports import InvoiceGlobalRemissionPreview
from ....reports import InvoiceGlobalRemissionPreviewM
from ....utils.converters import format_cc_or_nit

class InvoiceGlobalRemission(DocumentHeader):
    """InvoiceGlobalRemission as a public model class.
    note::
    """
    @staticmethod
    ...
    @staticmethod
    def get_all_remissions(**kwargs):
        """
        Allow searh all remissions
        :param kwargs:
        :return:
        """
        ...
        document_header = session.query(DocumentHeader)\
            ...
            DocumentType.shortWord == "RP",
            DocumentHeader.state == 1,
            DocumentHeader.providerId == provider_id
        ).all()
        return document_header

    @staticmethod
    def get_gremission(identifier):
        """
        Allow obtain a invoce remission according to identifier
        :param identifier: invoice indetifier
        :return: a json object with invoice information
        """
        g_remissions = session.query(DocumentHeader).filter(
            DocumentHeader.documentHeaderId == identifier
        ).first()
        return g_remissions

    @staticmethod
    def save_invoice_gremission(data, short_word):
        """
        Allaow save a global invoice
        :param data: global invoice remission information
        :param short_word: string identifier by global invoice document
        ...
        try:
            if 'documentDetails' in data and data['documentDetails'] is None and len(data['documentDetails']) == 0:
                abort(400)
            if short_word != "FP" and data['sourceShortWord'] != "FRP":
                abort(400)
            ...
            document_details = InvoiceGlobalRemission.import_details(document_detail_list)
            ...
            source_docId = session.query(DocumentType.documentTypeId).filter(
                DocumentType.shortWord == data['sourceShortWord']).scalar()
            ...
            InvoiceGlobalRemission.save_document_details(document_header, document_header_id, document_details)
            # Consulta la tasa de cambio y la guarda si ya hay una en esa fecha
            InvoiceGlobalRemission.save_exchange_rate(document_header)
            payment_recipt = None
             # Proceso de contabilizacion
            InvoiceGlobalRemission.save_accounting(document_header, document_details, payment_recipt)
            session.commit()
            # Actualiza el estado del documento origen
            InvoiceGlobalRemission.update_state_of_document(document_header, data, document_details)
            # session.expire_all()
    ...
    @staticmethod
    def update_invoice_gremission(id_invoice_gremission, data):
        """
        Allow update global invoice remission
        :param id_invoice_gremission:
        :param data:
        :return:
        """
        try:
            # Valida que el id enviado en la url sea el mismo enviado en el data
            if id_invoice_gremission != data['documentHeaderId']:
                abort(400)
            # Obtiene el documentheader por el id
            document_header = DocumentHeader.get_by_id(id_invoice_gremission)
            ...
            dds = DocumentDetail.get_document_details_by_document_header_id(id_invoice_gremission)
            # Actuliza seriales
            InvoiceGlobalRemission.update_serials(id_invoice_gremission)
            document_detail_list = data['documentDetails']
            # Actualiza detalles
            all_document_details = InvoiceGlobalRemission.update_document_details(document_header, data, dds, document_detail_list,
                                                                             id_invoice_gremission)
            # Guarda seriales si hay algo para guardar
            if any(i for i in all_document_details if len(i.listSerials)):
                for d in all_document_details:
                    for s in d.listSerials:
                        InvoiceGlobalRemission.save_serials(s, d)
            # Consulta el payment Receipt
            receipt_saved = PaymentReceipt.get_receipt_by_document_identifier(id_invoice_gremission)
            payment_receipt = data['paymentReceipt']
            if len(payment_receipt) > 0:
                payment_receipt = InvoiceGlobalRemission.update_payment_receipt(document_header, data,
                                                                         receipt_saved, payment_receipt,
                                                                                id_invoice_gremission)
            else:
                payment_receipt_saved = session.query(PaymentReceipt.paymentReceiptId) \
                    .filter(PaymentReceipt.documentHeaderId == id_invoice_gremission)
            ...
            InvoiceGlobalRemission.update_accounting(id_invoice_gremission, document_header, all_document_details, payment_receipt)
            session.commit()
            # Actualiza el estado del documento origen
            InvoiceGlobalRemission.update_state_of_document(document_header, data, all_document_details)
    ...
    def delete_invoice_gremission(id_invoice_gremission):
        """
        Allow delete a global invoice remission
        :param id_invoice_gremission: identifier by invoice remission
        ...
        try:
            ...
            document_header = session.query(DocumentHeader) \
                ...
                filter(DocumentHeader.documentHeaderId == id_invoice_gremission).first()
            if document_header is None:
                # abort(404)
            ...
            accounting_records = session.query(AccountingRecord) \
                .filter(or_(AccountingRecord.documentHeaderId == id_invoice_gremission,
                            AccountingRecord.crossDocumentHeaderId == id_invoice_gremission)).delete()
            session.flush()
            # Elimina seriales
    ...
    @staticmethod
    def get_invoice_g_remission_preview(document_header_id, format_type='P', document_type='D'):
        """
        ...
        try:
            if document_type == 'D':
                ...
                if format_type == 'P':
                    return InvoiceGlobalRemissionPreview.make_preview_pdf(formatted_data)
                elif format_type == 'M':
                    return InvoiceGlobalRemissionPreviewM.make_preview_pdf(formatted_data)
        ...
        for dd in document_details:
            ...
            if dd.listSerials is not None:
                for s in dd.listSerials:
                    InvoiceGlobalRemission.save_serials(s, dd)
    ...
    def save_accounting(document_header, document_details, payment_receipt):
        ...
        # Proceso de contabilizacion
        from ....models import InvoiceGlobalRemissionAccounting
        accounting = InvoiceGlobalRemissionAccounting(document_header)
    ...
    @staticmethod
    def update_serials(id_refund_provider):
        """
        ...
        serial_details = SerialDetail.get_serial_detail_by_document_header_id(id_refund_provider)
        [sd.delete() for sd in serial_details]
        # Elimina seriales de ultimo
        [s.delete() for s in Serial.get_serial_by_document_header_id(id_refund_provider)]
    ...
    def update_accounting(id_refund_provider, dh, all_document_details, payment_receipt):
        ...
        :return: None
        """
        accounting_records = InvoiceGlobalRemission.get_accounting_by_purchase_item_id(id_refund_provider)
        ...
        # Crea de nuevo la contabilidad si esta restaurando o regrabando el documento
            ...
            if accounting_records is not None:
                [ar.delete() for ar in accounting_records]
            # Proceso de contabilizacion
            InvoiceGlobalRemission.save_accounting(dh, all_document_details, payment_receipt)
