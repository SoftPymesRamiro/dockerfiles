...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = InvoiceProviderNotesPreviewM.preview_data
        ...
        #estilo total
        style_p_total_right = ParagraphStyle('Normal')
        style_p_total_right.fontName = 'Arial'
        style_p_total_right.fontSize = 7
        style_p_total_right.leading = 7
        style_p_total_right.alignment = TA_RIGHT

        style_p_total_left = ParagraphStyle('Normal')
        style_p_total_left.fontName = 'Arial'
        style_p_total_left.fontSize = 7
        style_p_total_left.leading = 7
        style_p_total_left.alignment = TA_LEFT
        ...
        def footer(cnv, last_value=2):
            ...
            if preview_data['sourceDocumentType'] == 'FP' or preview_data['sourceDocumentType'] == 'FRP' \
                    or preview_data['sourceDocumentType'] == 'FPA':
                ...
                table_footer_taxes = Table(
                    ...
                    [3.207 * cm, 3.107 * cm, 3.207 * cm, 3.207 * cm, 0.1 * cm],  # Ancho de las columnas
                    [0.4 * cm, 1.2 * cm],  # Alto de las filas
                ...
                table_footer_taxes.drawOn(cnv, 1 * cm, (last_value + 14.7) * cm)

            elif preview_data['sourceDocumentType'] == 'FPD' or preview_data['sourceDocumentType'] == 'FPI' \
                    or preview_data['sourceDocumentType'] == 'FPC':
                ...
                table_footer_taxes = Table(
                    data_footer_taxes,  # Contenido de la tabla
                    [2.206 * cm, 1 * cm, 1 * cm, 2.206 * cm],  # Ancho de las columnas
                    [0.4 * cm, 1.2 * cm],  # Alto de las filas
                    style=table_style_footer_taxes  # Estilos de la tabla
                )
                # Denota el contenor de la tabla
                table_footer_taxes.wrap(6.415 * cm, 2.6 * cm)
                # Dibuja la tabla en las coordenadas indicadas
                table_footer_taxes.drawOn(cnv, 7.4 * cm, (last_value + 14.7) * cm)
            ...
            # INFORMACIÓN SOBRE FACTUTACIÓN - INICIO ------------------------------------------------------------------
            if preview_data['sourceDocumentType'] == 'FPD' or preview_data['sourceDocumentType'] == 'FPI' \
                    or preview_data['sourceDocumentType'] == 'FPC':
                data_factura = [

                    [
                        Paragraph('<b>¿Qué le facturaron?</b>', style_p_footer_left),

                    ],
                    [
                        Paragraph('<b>{0} </b>{1}'.format('{0}{1}{2} {3} {4}'.format(preview_data['puc'].pucClass,
                                                                 preview_data['puc'].pucSubClass,
                                                                 preview_data['puc'].account,
                                                                 preview_data['puc'].subAccount,
                                                                 preview_data['puc'].auxiliary1),
                                                                    preview_data['puc'].name,),
                                  style_p_footer_left),
                ...
                table_factura = Table(
                    data_factura,  # Contenido de la tabla
                    [6.415 * cm],  # Ancho de las columnas
                    [0.5 * cm, 1.63 * cm],  # Alto de las filas
                    style=table_style_factura  # Estilos de la tabla
                )

                # Denota el contenor de la tabla
                table_factura.wrap(6.415 * cm, 2.6 * cm)
                # Dibuja la tabla en las coordenadas indicadas
                table_factura.drawOn(cnv, 1 * cm, (last_value + 14.7) * cm)


            # INFORMACIÓN SOBRE FACTURACIÓN - FIN ---------------------------------------------------------------------
            ...
            table_footer_totaling = Table(
                ...
                [0.35 * cm, 0.35 * cm, 0.35 * cm, 0.35 * cm, 0.35 * cm, 0.35 * cm, 0.35 * cm, 0.35 * cm,
                 0.35 * cm, 0.35 * cm, 0.35 * cm, 0.35 * cm, 0.6 * cm],
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 13.83 * cm, (last_value + 11.5) * cm)
            ...
            style_observaciones_extra.fontSize = 7
            style_observaciones_extra.leading = 7
            ...
            data_footer_observations = [
                [
                    Paragraph('<para><b>Observaciones:</b> {0} </para>'.format(par_observations),
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, (last_value + 13.5) * cm)
            ...
            data_footer_total_word = [
                [
                    Paragraph(par_total_word, style_observaciones_extra),
                ]
            ]
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, (last_value + 12.8) * cm)
            ...
            data_footer_sing = [
                [
                    Paragraph('Proveedor', style_p_footer_centred),
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_sing.drawOn(cnv, 1 * cm, (last_value + 11.5) * cm)
        ...
        def header(cnv):
            ...
            table_header_date_validity = Table(
                [
                    [
                        ...
                        Paragraph('<b>{0} No</b>'.format(preview_data['sourceDocumentType']),
                                  style_p_header_date_validity),
                        Paragraph('{0} {1}'.format(
                            '' if preview_data['sourceControlPrefix'] is None else preview_data['sourceControlPrefix'],
                            preview_data['sourceControlNumber']), style_p_header_date_validity),
                ...
                table_header_description = Table(
                    [
                        [
                            ...
                            Paragraph('<b>UND</b>', style_p_header_description_left),
                    ...
                    [1.9 * cm, 7 * cm, 1.2 * cm, 2 * cm, 2 * cm, 1.3 * cm, 1.2 * cm, 3 * cm],
                    [0.4 * cm],
            ...
            elif preview_data['sourceDocumentType'] == 'FPI':
                table_header_description = Table(
                    [
                        [

                            Paragraph('<b>DESCRIPCIÓN</b>', style_p_header_description_left),
                            Paragraph('<b>DOCUMENTO</b>', style_p_header_description_left),
                            Paragraph('<b>VENCE</b>', style_p_header_description_right),
                            Paragraph('<b>CANTIDAD</b>', style_p_header_description_right),
                            Paragraph('<b>%DSC</b>', style_p_header_description_right),
                            Paragraph('<b>%IVA</b>', style_p_header_description_right),
                            Paragraph('<b>VALOR</b>', style_p_header_description_right),
                    ...
                    [7 * cm, 2.1 * cm, 3 * cm, 2 * cm, 1.3 * cm, 1.2 * cm, 3 * cm],
                    [0.4 * cm],
                    style=table_style_header_description
                )
            ...
            elif preview_data['sourceDocumentType'] == 'FPA':
                table_header_description = Table(
                    [
                        [
                            Paragraph('<b>CÓDIGO</b>', style_p_header_description_left),
            ...
            elif preview_data['sourceDocumentType'] == 'FPD':
                table_header_description = Table(
                    [
                        [

                            Paragraph('<b>DESCRIPCIÓN</b>', style_p_header_description_left),
                            Paragraph('<b>MESES</b>', style_p_header_description_left),
                            Paragraph('<b>%DESCUENTO</b>', style_p_header_description_right),
                            Paragraph('<b>%IVA</b>', style_p_header_description_right),
                            Paragraph('<b>VALOR</b>', style_p_header_description_right),
                        ]
                    ...
                    [10.3 * cm, 2 * cm, 2.3 * cm, 2 * cm, 3 * cm],  # ancho columnas
            ...
            elif preview_data['sourceDocumentType'] == 'FPC':
                table_header_description = Table(
                    [
                        [
                    ...
                    [12.3 * cm, 2.3 * cm, 2 * cm, 3 * cm],  # ancho columnas
...
class InvoiceProviderNotesPreviewM:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        InvoiceProviderNotesPreviewM.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
        ...
        if preview_data['sourceDocumentType'] == 'FP' or preview_data['sourceDocumentType'] == 'FRP':
            ...
            for dct_det in preview_data['document_details']:
                ...
                if dct_det.sourceDocumentDetail and dct_det.sourceDocumentDetail.documentHeader.documentType.shortWord != 'RP':
                    ...
                    if dct_det.lot is not None and dct_det.dueDate is not None:
            ...
            # for i in range(20):
                data.append(
                    [
                        ...
                        Paragraph('{0}{1}{2}{3}{4}{5}'.format(dct_det.item.name,
                                                              det_siz_col_brake,
                                                              det_size,
                                                              det_color,
                                                              det_lot,
            ...
            for dct_det in preview_data['document_details']:
                ...
                data.append(
                    [
                        Paragraph("{0} {1} {2} {3}".format("" if dct_det.third.tradeName is None
                                                           else dct_det.third.tradeName.strip(),
                                                           "" if dct_det.third.lastName is None
                                                           else dct_det.third.lastName.strip(),
                                                           "" if dct_det.third.maidenName is None
                                                           else dct_det.third.maidenName.strip(),
                                                           "" if dct_det.third.firstName is None
                                                           else dct_det.third.firstName.strip()), style),
                        Paragraph(dct_det.detailDocument, style),
                        Paragraph(dct_det.dueDate.strftime('%d %b. %Y'), style_num),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.quantity, default_decimals.quantityDecimals),
                                                      default_decimals.quantityDecimals), style=style_num),
            ...
            for dct_det in preview_data['document_details']:
                ...
                data.append(
                    [
                        ...
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.quantity, default_decimals.quantityDecimals),
                                                      default_decimals.quantityDecimals), style=style),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.disccount, 2), 2),
                                  style=style_num),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.iva, 2), 2), style=style_num),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.value, default_decimals.valueDecimals),
            ...
            for dct_det in preview_data['document_details']:
                ...
                data.append(
                    [
                        Paragraph(dct_det.puc.name, style),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.disccount, 2), 2),
                                  style=style_num),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.iva, 2), 2), style=style_num),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.value, default_decimals.valueDecimals),
        ...
        if preview_data['sourceDocumentType'] == 'FP' or preview_data['sourceDocumentType'] == 'FRP':
            ...
            doc.build(
                [Table(
                    data,
                    [2 * cm, 7 * cm, 1.1 * cm, 2 * cm, 2 * cm, 1.3 * cm, 1.2 * cm, 3 * cm],
                    style=table_style
                )],
            ...
            doc.build(
                [Table(
                    data,
                    [7.1 * cm, 2 * cm, 3 * cm, 2 * cm, 1.3 * cm, 1.2 * cm, 3 * cm],
                    style=table_style
                )],
                canvasmaker=NumberedCanvas
            )
            ...
            doc.build(
                [Table(
                    data,
                    [10.4 * cm, 1.9 * cm, 2.3 * cm, 2 * cm, 3 * cm],
                    style=table_style
                )],
                canvasmaker=NumberedCanvas
            )
