...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = InvoicePurchaseItemPreview_M.preview_data
        ...
        def footer(cnv):
            ...
            data_footer_totaling = [
                ...
                #     Paragraph('', style_p_footer_right),
                #     Paragraph('{:20,.{}f}'.format(_round(base_gravable, default_decimals.valueDecimals),
                #                                   default_decimals.valueDecimals), style_p_footer_right)
                # ],
                # [
            ...
            table_footer_totaling = Table(
                ...
                [2.86 * cm, 1.4 * cm, 2.75 * cm],  # Ancho de las columnas
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 13.7 * cm, 14.3 * cm)

            # cnv.line(13.7 * cm, 30 * cm, 13.7 * cm, 1 * cm) #linea de medida
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 16.2 * cm)
            ...
            # Denota el contenor de la tabla
            table_footer_sing.wrap(12.7 * cm, 2.583 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_sing.drawOn(cnv, 1 * cm, 14.3 * cm)
        ...
        def header(cnv):
            ...
            table_header_provider = Table(
                [
                    ...
                    [
                        ...
                        Paragraph(paragraph_over_flow(text=preview_data['provider_cellPhone'],
                                                      width=4,
                                                      font_size=8,
                                                      leading=8),
                                  style_p_header_provider),
            ...
            table_header_date_validity.wrap(6.9 * cm, 0.6 * cm)
...
class InvoicePurchaseItemPreview_M:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        InvoicePurchaseItemPreview_M.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
        ...
        for dct_det in preview_data['document_details']:
            ...
            det_siz_col_brake = ''
            det_serials = ''
            if dct_det.sourceDocumentDetail is None \
                    or (dct_det.sourceDocumentDetail and dct_det.sourceDocumentDetail.documentHeader.documentType.shortWord != 'RP'):
