...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = ClosePurchaseImportPreview.preview_data
        ...
        def footer(cnv, last_value= 7):
            ...
            data_footer_totaling = [
                ...
                [
                    Paragraph('<b>Gastos a Distribuidor</b>', style_p_footer),
                    Paragraph('', style_p_footer_right),
                    Paragraph('${:20,.{}f}'.format(_round(preview_data['expenses'], default_decimals.valueDecimals),
                                                  default_decimals.valueDecimals), style_p_footer_right)
                ],
                [
            ...
            # texto = '<para><b>Observaciones:</b>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. ' \
            #         'Aenean commodo ligula eget dolor. Aenean massa. ' \
            #         'Cum sociis natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus. ' \
            #         'Donec quam felis</para>'
        ...
        def header(cnv):
            ...
            data_header_company = [
                ...
                [
                    Paragraph('<b>{0}</b>'.format(
                        paragraph_over_flow(
                            text='LEGALIZACIÓN DE IMPORTACIÓN',
                            width=12.8,
                            font_size=12,
                            leading=12)),
                        style_p_header),
            ...
            table_header_date_validity = Table(
                [
                    ...
                    [
                        Paragraph('<b>Importación: </b>'+ preview_data['import_code'], style_p_header_date_validity),
                ...
                [4 * cm, 15.6 * cm],
                [0.2 * cm, 0.5 * cm, 0.1 * cm, 0.5 * cm, 0.2 * cm],
                # 0.4 * cm,
            ...
            if preview_data['document_details'][0].item is not None:
                table_header_description = Table(
                       [
                           [
                               ...
                               Paragraph('<b>VALOR UNIT</b>', style_p_header_description_right),
                ...
                table_header_description = Table(
                      [
                          [
                              Paragraph('<b>CODIGO</b>', style_p_header_provider),
                              Paragraph('<b>DESCRIPCION</b>', style_p_header_provider),
                              Paragraph('<b>VALOR</b>', style_p_header_description_right),
            ...
            #
            cnv.line(1 * cm, 22 * cm, 20.6 * cm, 22 * cm)  # Línea de firma proveedor
...
class ClosePurchaseImportPreview:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        ClosePurchaseImportPreview.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
        ...
        if preview_data['document_details'][0].asset is not None:
            ...
            doc.build(
                [Table(
                    data,
                    [1.9 * cm, 9 * cm, 1.1 * cm, 2.3 * cm, 2.6 * cm, 2.7 * cm],
                    style=table_style
                )],
                canvasmaker=NumberedCanvas
            )
