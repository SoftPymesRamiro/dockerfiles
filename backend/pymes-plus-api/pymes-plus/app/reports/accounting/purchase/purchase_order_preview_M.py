...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = PurchaseOrderPreviewM.preview_data
        ...
        def footer(cnv):
            ...
            table_footer_send = Table(
                ...
                [2 * cm, 5.35 * cm, 1.8 * cm, 5.35 * cm],  # Ancho de las columnas
                style=table_style_footer_general  # Estilos de la tabla
            )

            # Denota el contenor de la tabla
            table_footer_send.wrap(13 * cm, 2.6 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_send.drawOn(cnv, 1 * cm, 17.35 * cm)
            ...
            if page_count == self._pageNumber:
                ...
                for tax in tax_values:
                    if len(base_value_arr) < 7:
                        base_value_arr.append(locale.format(
                            "%.{0}f".format(default_decimals.valueDecimals),
                            _round(tax['base_value'], default_decimals.valueDecimals),
                            grouping=True))
                    if len(iva_percent_arr) < 7:
                        iva_percent_arr.append(locale.format(
                            "%.{0}f".format(2),
                            _round(tax['iva_percent'], 2),
                            grouping=True))
                    if len(ic_percent_arr) < 7:
                        ic_percent_arr.append(locale.format(
                            "%.{0}f".format(2),
                            _round(tax['ic_percent'], 2),
                            grouping=True))
                    if len(tax_value_arr) < 7:
                        tax_value_arr.append(locale.format(
                            "%.{0}f".format(default_decimals.valueDecimals),
                            _round(tax['tax_value'], default_decimals.valueDecimals),
                            grouping=True))
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_taxes.drawOn(cnv, 7.5 * cm, 6.7 * cm)
            """
            ...
            data_footer_totaling = [
                [
                    Paragraph('<strong>SUBTOTAL</strong>', style_p_footer),
                ...
                [
                    Paragraph('-Descuento', style_p_footer),
                    Paragraph('${:20,.{}f}'.format(_round(descuento, default_decimals.valueDecimals),
                                                  default_decimals.valueDecimals), style_p_footer_right)
                ],
                [
                    Paragraph('-Otros Dsctos', style_p_footer),
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 15.5 * cm, 14.45 * cm)

            # cnv.line(15.5 * cm, 30 * cm, 15.5 * cm, 3 * cm)  # Linea base
            ...
            data_footer_observations = [
                [
                    Paragraph('<b>Observaciones:</b> {0} '.format(par_observations), style_p_footer),
                ]
            ]
            ...
            # Denota el contenor de la tabla
            table_footer_observations.wrap(13 * cm, 1.1 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 16.25 * cm)
            ...
            par_total_word = paragraph_over_flow_height(text=tot_word, width=13, no_par=2, font_size=8, leading=8)
            ...
            # Denota el contenor de la tabla
            table_footer_total_word.wrap(13 * cm, 0.8 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 15.45 * cm)
            ...
            table_footer_sing = Table(
                ...
                [7.25 * cm, 7.25 * cm],  # Ancho de las columnas
                [1 * cm],
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_sing.drawOn(cnv, 1 * cm, 14.45 * cm)
            ...
            data_header_company = [
                ...
                [
                    Paragraph('<b>NIT: {0}-{1} {2}</b>'.format(
                        paragraph_over_flow(
                            # text='<b>NIT:</b> {0} {1}'.format(preview_data['nit'], preview_data['regimen']),
                ...
                [
                    ...
                    [
                        ...
                        Paragraph(paragraph_over_flow(
                            text='.'.join([str(nit1[0])[::-1][i:i + 3] for i in range(0, len(str(nit1[0])), 3)])[
                                 ::-1] + '-' + nit1[1],
                            width=4.3,
                            font_size=8,
                            leading=8),
...
class PurchaseOrderPreviewM:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        PurchaseOrderPreviewM.preview_data = preview_data
        ...
        style1 = ParagraphStyle('Normal')
        style_num = ParagraphStyle('Normal')

        style1.backColor = '#FFFF00'
        style1.borderColor = '#000000'
        style1.borderWidth = 1
        ...
        doc.build(
            [Table(
                data,
                [2.3 * cm, 7 * cm, 1.1 * cm, 1.7 * cm, 2 * cm, 1.3 * cm, 1.2 * cm, 3* cm],
                style=table_style
            )],
