...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = InvoicePurchaseItemPreview.preview_data
        ...
        def footer(cnv):
            ...
            table_footer_taxes = Table(
                ...
                [3 * cm, 2.6 * cm, 2 * cm, 3 * cm, 2.1 * cm],  # Ancho de las columnas
...
class InvoicePurchaseItemPreview:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        InvoicePurchaseItemPreview.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
        ...
        for dct_det in preview_data['document_details']:
            ...
            det_siz_col_brake = ''
            det_serials = ''
            # if dct_det.sourceDocumentDetail and dct_det.documentHeader.documentType.shortWord != 'RP':
        ...
        doc.build(
            [Table(
                data,
                [2.1 * cm, 7 * cm, 1.2 * cm, 1.7 * cm, 2.1 * cm, 1.2 * cm, 1.2 * cm, 3 * cm],
                style=table_style
            )],
