...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = InvoicePurchaseImportOutTimePreview.preview_data
        ...
        table_style_header_count = TableStyle(
            [
                ('LEFTPADDING', (0, 0), (-1, -1), 0.1 * cm),
                # ('LEFTPADDING', (1, 0), (-1, 0), 0),
                ('RIGHTPADDING', (0, 0), (-1, -1), 0.01 * cm),
        ...
        style_p_header_count = ParagraphStyle('Normal')
        style_p_header_count.fontName = 'Arial'
        style_p_header_count.fontSize = 8
        style_p_header_count.leading = 8
        ...
        def footer(cnv, last_value=2):
            ...
            #     [2.3 * cm, 10.53 * cm],  # Ancho de las columnas
            #     [0.5 * cm, 0.5 * cm, 0.5 * cm, 0.48 * cm],  # Alto de las filas
            ...
            # table_footer_taxes.drawOn(cnv, 1 * cm,
            #                           (14.18 - last_value) * cm)
            ...
            #     directIva = _round(preview_data['directIVA'], default_decimals.valueDecimals)
            ...
            #         Paragraph('<b>'+'${:20,.{}f}'.format(_round(sub_total, default_decimals.valueDecimals),
            #                                       default_decimals.valueDecimals)+'</b>', style_p_footer_right)
            #     ],
            #     [
            ...
            #     ],
            #     [
            #         Paragraph('<b>- Otros Descuentos</b>', style_p_footer),
            ...
            #         Paragraph('', style_p_footer_right),
            #         Paragraph('<b>'+'${:20,.{}f}'.format(_round(base_gravable, default_decimals.valueDecimals),
            #                                       default_decimals.valueDecimals)+'</b>', style_p_footer_right)
            #     ],
            #     [
            ...
            #
            #     [
            #         Paragraph('<b>+ IVA Directo</b>', style_p_footer),
            #         Paragraph('', style_p_footer_right),
            #         Paragraph('${:20,.{}f}'.format(_round(directIva, default_decimals.valueDecimals),
            #                                       default_decimals.valueDecimals), style_p_footer_right)
            #     ],
            #     [
            ...
            #         Paragraph('0.00%' if (rete_iva_percent == '' or rete_iva_percent == 0) else "{0}%".format(
            ...
            #
            #     [2.86 * cm, 1.22 * cm, 2.75 * cm],# Ancho de las celdas
            ...
            # # Dibuja la tabla en las coordenadas indicadas
            # table_footer_totaling.drawOn(cnv, 13.83 * cm, (10.9 - last_value) * cm)
            # #linea total
            # cnv.line(18 * cm, 9.3 * cm, 20.7 * cm, 9.3 * cm)


            #linea guia
            # cnv.line(13.83 * cm, 15 * cm, 13.83 * cm, 13 * cm)
            ...
            # # Dibuja la tabla en las coordenadas indicadas
            # table_footer_observations.drawOn(cnv, 1 * cm, (11.86 - last_value) * cm)
            ...
            #
            # # Denota el contenor de la tabla
            # table_footer_total_word.wrap(12.7 * cm, 6.7 * cm)
            # # Dibuja la tabla en las coordenadas indicadas
            # table_footer_total_word.drawOn(cnv, 1 * cm, (10.9 - last_value) * cm)
        ...
        def header(cnv):
            ...
            nit_head = preview_data['nit'].split('-')
            data_header_company = [
                [
                    p_company_name,
                ],
                [
                    ...
                    Paragraph('<b>NIT: {0}-{1} {2}</b>'.format(
                        paragraph_over_flow(
                            ...
                            text='.'.join([str(nit_head[0])[i:i + 3]
                                           for i in range(0, len(str(nit_head[0])), 3)]),
                            width=12.8,
                            font_size=12,
                            leading=12), nit_head[1], preview_data['regimen']),
                        style_p_header),
            ...
            table_header_provider = Table(
                [
                    [
                        ...
                        Paragraph(
                            paragraph_over_flow(text=preview_data['provider'],
                                                width=10,
                                                font_size=8,
                                                leading=8),
                    ...
                    [
                        ...
                        Paragraph(paragraph_over_flow(
                            text='.'.join([str(nit1[0])[::-1][i:i + 3] for i in range(0, len(str(nit1[0])), 3)])[::-1] + '-' + nit1[1],
                            width=4,
                            font_size=8,
                            leading=8),
                            style_p_header_provider)
                    ...
                    [
                        ...
                        Paragraph(paragraph_over_flow(text='' if preview_data['provider_email'] is None else preview_data['provider_email'],
                                                      width=4,
                                                      font_size=8,
                                                      leading=8),
                                  style_p_header_provider),
                ...
                [1.6 * cm, 4.33 * cm, 2 * cm, 4.9 * cm],
                0.4 * cm,
                style=table_style_header_general
            )
            table_header_provider.wrap(13 * cm, 2 * cm)
            ...
            table_header_date_validity = Table(
                [
                    ...
                    [
                        ...
                        Paragraph("{0}".format(preview_data['consecutive']),
            ...
            table_header_date_validity.drawOn(cnv, 13.83 * cm, 20.68 * cm)
            ...
            table_header_date_validity = Table(
                [
                    ...
                    [
                        Paragraph('<b>Importación</b>', style_p_header_count),
                        Paragraph("{0} {1}".format(preview_data['import_code'], preview_data['import_name']),
                                  style_p_header_count),
                        '',
                        ''
                    ],
                    [
                        Paragraph('<b>Moneda</b>', style_p_header_count),
                        Paragraph(preview_data['currency'], style_p_header_count),
                        Paragraph('<b>Tasa de Cambio</b>', style_p_header_count),
                        Paragraph('{:20,.{}f}'.format(_round(preview_data['exchangeRate'], 2), 2),
                                  style_p_header_count),
                    ],
                    [
                        Paragraph('', style_p_header_date_validity),
                ...
                [2 * cm, 4 * cm, 2.6 * cm, 11 * cm],
                0.4 * cm,
                style=table_style_header_count
            )
            table_header_date_validity.wrap(4 * cm, 0.6 * cm)
            table_header_date_validity.drawOn(cnv, 1 * cm, 19.08 * cm)
            ...
            table_header_description = Table(
                [
                    [
                        Paragraph('<b>CONCEPTO</b>', style_p_header_provider),
                        Paragraph('<b>%DSC</b>', style_p_header_description_right),
                        Paragraph('<b>%IVA</b>', style_p_header_description_right),
                        Paragraph('<b>%RF</b>', style_p_header_description_right),
                        Paragraph(new_value, style_p_header_description_right),
                        Paragraph('<b>VALOR</b>', style_p_header_description_right),
                    ]

                ],
                [9 * cm, 1.8 * cm, 1.4 * cm, 1.4 * cm, 3 * cm, 3 * cm],
                [0.4 * cm],
                style=table_style_header_description
            )

            table_header_description.wrap(19.6 * cm, 0.4 * cm)
            table_header_description.drawOn(cnv, 1 * cm, 18.7 * cm)
            #
            cnv.line(1 * cm, 18.66 * cm, 20.6 * cm, 18.66 * cm)  # Línea de firma proveedor
            ...
            if preview_data['provider_iva'] == 'S':
                ...
                document = preview_data['document_type'].split(' ')
                document_type = document[0] + ' ' + document[1] + ' ' + 'IMPORTACIÓN' if document[2] == 'IMPORTACION' \
                    else document[2]
            ...
            table_header_order_no.drawOn(cnv, 13.83 * cm, 22.27 * cm)
...
class InvoicePurchaseImportOutTimePreview:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        InvoicePurchaseImportOutTimePreview.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
        ...
        for dct_det in preview_data['document_details']:
            data.append(
                [
                    Paragraph(dct_det.importConcept.name, style),
                    ...
                    Paragraph('{:20,.{}f}'.format(_round(dct_det.withholdingTax, 2), 2), style=style_num),
                    Paragraph('{:20,.{}f}'.format(_round(dct_det.baseValue / preview_data['exchangeRate'], 2), 2),
                              style=style_num),
        ...
        #   TABLA DETALLATES TOTALES -- INICIO------------------------------------------------------------------------------------------------------------------------
        ...
        table_style_footer_total = TableStyle(
            [
                ('LEFTPADDING', (0, 0), (0, -1), 0.1 * cm),
                ...
                ('BOX', (0, 0), (-1, -1), 1, (0, 0, 0)),
                ('VALIGN', (0, 4), (-1, -1), 'TOP'),
                ('LINEBEFORE', (1, 0), (1, -1), 1, (0, 0, 0)),  # linea vertical
                ('LINEABOVE', (0, 4), (0, 4), 1, (0, 0, 0)),  # linea horizontal
                ('LINEABOVE', (0, 7), (0, -2), 1, (0, 0, 0)),  # linea horizontal
                # ('SPAN', (0, 0), (0, 3)),
                ('SPAN', (0, 4), (0, 7)),
                ('SPAN', (0, 8), (0, 9)),
                # ('GRID', (0, 0), (-1, -1), 1, (0, 0, 0))
        ...
        que_factura = '<para><b>{0}</b> {1} </para>'.format(preview_data['import_account'],
                                                            preview_data['import_puc_name'])

        cuenta_puc = paragraph_over_flow_height(text=que_factura,
                                                width=13,
                                                no_par=5,
                                                font_size=8,
                                                leading=8)
        ...
        data_footer_totaling = [
            [
                Paragraph(cuenta_puc, style_p_footer),
                Paragraph('<b>SUBTOTAL</b>', style_p_footer),
                Paragraph('', style_p_footer_right),
                Paragraph('<b>' + '${:20,.{}f}'.format(_round(sub_total, default_decimals.valueDecimals),
                                                       default_decimals.valueDecimals) + '</b>', style_p_footer_right)
        ...
        table_footer_totaling = Table(
            ...
            [12.83 * cm, 2.86 * cm, 1.22 * cm, 2.75 * cm],
            style=table_style_footer_total  # Estilos de la tabla
        )

        #   TABLA DETALLATES TOTALES -- FIN---------------------------------------------------------------------------------------------------------------------------
