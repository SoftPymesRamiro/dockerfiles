...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = InvoiceGlobalRemissionPreviewM.preview_data
        ...
        def footer(cnv):
            ...
            # data_footer_send = [
            #         [
            #             Paragraph('<b>Despachar a</b>', style_p_footer),
            ...
            #         ],
            #         [
            #             Paragraph('<b>NIT:</b>', style_p_footer),
            ...
            #         ],
            #         [
            #             Paragraph('<b>E-mail:</b>', style_p_footer),
            #             Paragraph(paragraph_over_flow(text='',
            #                                           width=4.7,
            #                                           font_size=8,
            #                                           leading=8), style_p_footer)
            ...
            #     [1.8 * cm, 4.7 * cm],  # Ancho de las columnas
            ...
            #             base_value_arr.append('{:20,.{}f}'.format(
            #                 _round(tax['base_value'], default_decimals.valueDecimals), default_decimals.valueDecimals))
            #         if len(iva_percent_arr) < 7:
            #             iva_percent_arr.append('{:20,.{}f}'.format(_round(tax['iva_percent'], 2), 2))
            #         if len(ic_percent_arr) < 7:
            #             ic_percent_arr.append('{:20,.{}f}'.format(
            #                 _round(tax['ic_percent'], 2), 2))
            ...
            #
            #     [
            #         Paragraph('<b>Vr. Base</b>', style_p_footer_centred),
            ...
            #     ],
            #     [
            #         # Paragraph(base_value_par, style_p_footer_right), original
            ...
            #     [4 * cm, 1.6 * cm, 1.7 * cm, 5.4 * cm],  # Ancho de las columnas
            ...
            data_footer_totaling = [
                ...
                [
                    Paragraph('<b>- Impuesto CREE</b>', style_p_footer),
                    Paragraph('', style_p_footer_right),
            ...
            table_footer_totaling = Table(
                ...
                [2.86 * cm, 0.98 * cm, 2.75 * cm],  # Ancho de las columnas
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 14 * cm, 14.2 * cm)

            # linea de medicion
            # cnv.line(13.7 * cm, 30 * cm, 13.7 * cm, 3 * cm) # //////////////////////////////////////////////////
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 16.68 * cm)
            ...
            # Denota el contenor de la tabla
            table_footer_total_word.wrap(13 * cm, 1.1 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 15.48 * cm)
            ...
            table_footer_sing = Table(
                ...
                [6.5 * cm, 6.5 * cm],  # Ancho de las columnas
                [1.28 * cm],
                style=table_style_footer_sing  # Estilos de la tabla
            )

            # Denota el contenor de la tabla
            table_footer_sing.wrap(13 * cm, 2.6 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_sing.drawOn(cnv, 1 * cm, 14.2 * cm)
        ...
        def header(cnv):
            ...
            table_header_provider.wrap(12.36 * cm, 2 * cm)
            ...
            if preview_data['currencyId'] != default_decimals.currencyId:
                text_trm = "<b>Tasa de Cambio</b> {0}".format(_round(preview_data['exchangeRate'], 2), 2)
                moneda = '<b>Moneda</b>'
                moneda_value = preview_data['currency'].title()
            else:
                text_trm = ''
                moneda_value = ''
                moneda = ''
            ...
            table_header_date_validity = Table(
                [
                    ...
                    [
                        ...
                        Paragraph(moneda_value, style_p_header_date_validity),
            ...
            table_header_description = Table(
                [
                    [
                        Paragraph('<b>REMISIÓN</b>', style_p_header_provider),
                        Paragraph('<b>FECHA</b>', style_p_header_provider),
                        Paragraph('<b>ORDEN</b>', style_p_header_provider),
                ...
                [1.794 * cm, 1.664 * cm, 1.494 * cm, 1.594 * cm, 3.094 * cm, 1.094 * cm, 1.994 * cm, 1.994 * cm,
                 1.094 * cm, 1.094 * cm, 2.694 * cm],
                [0.4 * cm],
            ...
            if preview_data['provider_iva'] == 'S':
                document_type = 'DOCUMENTO EQUIVALENTE A FACTURA'
            else:
            ...
            table_header_order_no = Table(
                [
                    [
                        Paragraph("<b>{0}<br/>{1}{2}</b>".format(document_type, document_number_text, document_number),
                                  style_p_header_provider),
                    ],
                    [
            ...
            table_header_order_no.drawOn(cnv, 14 * cm, 22.67 * cm)

            # INFORMACION DE LA TABLA ORDEN DE COMPRA No - FIN --------------------------------------------------------

            # INFORMACION DE LA TABLA LOGO ----------------------------------------------------------------------------
            image = None if preview_data['image'] is None \
                else "{0},{1}".format("data:image/*;base64", ImagesConverter.img_convert_to_base64(preview_data['image'].image))
...
class InvoiceGlobalRemissionPreviewM:
    preview_data = None
    @staticmethod
    def make_preview_pdf(preview_data):
        InvoiceGlobalRemissionPreviewM.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
        ...
        for dct_det in preview_data['document_details']:
            ...
            det_siz_col_brake = ''
            det_serials = ''
            if not dct_det.sourceDocumentDetail:
            ...
            data.append(
                [
                    ...
                    Paragraph('' if not dct_det.sourceDocumentDetail else
                              dct_det.sourceDocumentDetail.documentHeader.documentDate.strftime("%d/%m/%Y"), style),
                    Paragraph(
                        dct_det.sourceDocumentDetail.documentHeader.orderNumber if dct_det.sourceDocumentDetail and
                                                                                   dct_det.sourceDocumentDetail.documentHeader.orderNumber
                        else '', style),
        ...
        doc.build(
            [Table(
                data,
                [1.8 * cm, 1.75 * cm, 1.5 * cm, 1.62 * cm, 3.094 * cm, 1.094 * cm, 1.88 * cm, 1.994 * cm,
                 1.094 * cm, 1.094 * cm, 2.694 * cm],
                style=table_style
            )],
