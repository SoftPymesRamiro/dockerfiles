...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = InvoicePurchaseImportsPreview.preview_data
        ...
        table_style_header_date_validity_principal = TableStyle(
            [
                ('LEFTPADDING', (0, 0), (-1, -1), 0.1 * cm),
                # ('LEFTPADDING', (1, 0), (-1, 0), 0),
                ('RIGHTPADDING', (0, 0), (-1, -1), 0.01 * cm),
                ...
                # ('VALIGN', (-2, 0), (-1, -1), 'MIDDLE'),
                ('SPAN', (2, 1), (3, 1)),
                # ('BACKGROUND', (2, 1), (3, 1), colors.orange),
                ('SPAN', (2, 2), (1, 2)),
                # ('GRID', (0, 0), (-1, -1), 1, (0, 0, 0)),
        ...
        def footer(cnv, last_value= -2):
            ...
            table_footer_taxes = Table(
                ...
                [0.5 * cm, 0.5 * cm, 0.5 * cm, 0.5 * cm],  # Alto de las filas
            ...
            # table_footer_taxes.drawOn(cnv, 1 * cm, (last_value+14.45) * cm)
            ...
            # # Dibuja la tabla en las coordenadas indicadas
            # # table_footer_totaling.drawOn(cnv, 13.7 * cm, (last_value+3.43) * cm) original
            # table_footer_totaling.drawOn(cnv, 13.83 * cm, (last_value + 11.2) * cm)
            # # table_footer_totaling.drawOn(cnv, 1 * cm, 5.7 * cm)
            # #linea total
            # cnv.line(18 * cm, (last_value + 11.7) * cm, 20.5 * cm, (last_value + 11.7) * cm)
            ...
            # # Dibuja la tabla en las coordenadas indicadas
            # table_footer_observations.drawOn(cnv, 1 * cm, (last_value+12.14) * cm)
            ...
            # # Dibuja la tabla en las coordenadas indicadas
            # table_footer_total_word.drawOn(cnv, 1 * cm, (last_value+11.20) * cm)
        ...
        def header(cnv):
            ...
            table_header_provider = Table(
                [
                    [
                        ...
                        Paragraph(
                            paragraph_over_flow(text=preview_data['provider'], width=10, font_size=8, leading=8),
            ...
            # INFORMACION DE LA TABLA PRINCIPAL - INICIO -----------------------------------------------------------
            ...
            table_header_date_validity = Table(
                [
                    ...
                    [
                        Paragraph('<b>Importación</b>', style_p_header_date_validity),
                    ...
                    [
                        ...
                        Paragraph(preview_data['currency'], style_p_header_date_validity),
                        Paragraph('<b>Tasa de Cambio</b>', style_p_header_date_validity),
            ...
            table_header_date_validity.drawOn(cnv, 1 * cm, 19.48 * cm)
            ...
            table_header_description = Table(
                [
                    [
                        ...
                        # Paragraph('<b></b>', style_p_header_provider),
                        # Paragraph('<b></b>', style_p_header_provider),
                ...
                [10 * cm, 1.8 * cm, 1.4 * cm, 1.4 * cm, 3 * cm, 2 * cm],
                [0.4 * cm],
                style=table_style_header_description
            )

            table_header_description.wrap(19.6 * cm, 0.4 * cm)
            table_header_description.drawOn(cnv, 1 * cm, 19 * cm)
            #
            cnv.line(1 * cm, 19 * cm, 20.6 * cm, 19 * cm)  # Línea de firma proveedor
            ...
            if preview_data['provider_iva'] == 'S':
                document_type = 'FACTURA DE IMPORTACIÓN'
            else:
...
class InvoicePurchaseImportsPreview:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        InvoicePurchaseImportsPreview.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
