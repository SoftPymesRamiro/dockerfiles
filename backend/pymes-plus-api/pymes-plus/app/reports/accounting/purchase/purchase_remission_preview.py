...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = PurchaseRemissionPreview.preview_data
        ...
        table_style_footer_totaling = TableStyle(
            [
                ...
                # Todas las celdas menos la ultima
                ('TOPPADDING', (0, 1), (-1, -2), 0.5 * mm),
                # Primera celda
                ('TOPPADDING', (0, 0), (-1, 0), 0.531 * mm),
                # Ultima celda
                ('TOPPADDING', (0, 7), (-1, -1), 0.2 * cm),
                ('BOX', (0, 0), (-1, -1), 1, (0, 0, 0)),
                ('VALIGN', (0, 0), (-1, -1), 'TOP'),
                # ('GRID', (0, 0), (-1, -1), 1, (0, 0, 0))
            ]
        ...
        table_style_header_order_no = TableStyle(
            [
                ('LEFTPADDING', (0, 0), (-1, 0), 1.25 * cm),
                # ('RIGHTPADDING', (0, 0), (-1, -1), 0.1 * cm),
                ('RIGHTPADDING', (0, 0), (-1, 0), 1.25 * cm),
        ...
        def footer(cnv):
            ...
            data_footer_send = [
                [
                    ...
                    Paragraph(paragraph_over_flow_height(text=preview_data['shipTo'].upper(),
                                                         width=4,
                                                         no_par=1,
                                                         font_size=8,
                ...
                [
                    Paragraph('<b>País:</b>', style_p_footer),
            ...
            table_footer_send = Table(
                ...
                [2.2 * cm, 4.04 * cm],  # Ancho de las columnas
                [0.446 * cm, 0.446 * cm, 0.446 * cm, 0.446 * cm, 0.446 * cm, 0.446 * cm],
            ...
            table_footer_send.wrap(6.5 * cm, 2.46 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_send.drawOn(cnv, 1 * cm, 4.406 * cm)
            ...
            data_footer_taxes = [
                ...
                [
                    Paragraph(base_value_par, style_p_footer_centred),
                    Paragraph(iva_percent_par, style_p_footer_centred),
                    Paragraph(ic_percent_par, style_p_footer_centred),
                    Paragraph(tax_value_par, style_p_footer_centred)
                ]
            ]

            table_footer_taxes = Table(
                data_footer_taxes,  # Contenido de la tabla
                [2.4 * cm, 1 * cm, 1 * cm, 2.34 * cm],  # Ancho de las columnas
                [0.5 * cm, 2.18 * cm],  # Alto de las filas
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_taxes.drawOn(cnv, 7.25 * cm, 4.406 * cm)
            ...
            data_footer_totaling = [
                ...
                [
                    Paragraph('<b>NETO REMISIÓN</b>', style_p_footer),
                    Paragraph('', style_p_footer_right),
                    Paragraph('<b>${0}</b>'.format('{:20,.{}f}'.format(_round(neto_a_pagar, default_decimals.valueDecimals),
                                                                       default_decimals.valueDecimals)), style_p_footer_right)
                ]
            ]
            ...
            table_footer_totaling = Table(
                ...
                [2.84 * cm, 1 * cm, 2.75 * cm],  # Ancho de las columnas
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 14 * cm, 3.4 * cm)

            # cnv.line(14 * cm, 30 * cm, 14 * cm, 3 * cm)  # Linea base
            ...
            # Denota el contenor de la tabla
            table_footer_observations.wrap(13 * cm, 1.6 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 3.408 * cm)
            ...
            par_total_word = paragraph_over_flow_height(text=tot_word, width=19, no_par=3, font_size=8, leading=8)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 2.4 * cm)
            ...
            table_footer_sing = Table(
                ...
                [9.795 * cm, 9.795 * cm],  # Ancho de las columnas
                [1.4 * cm],
        ...
        def header(cnv):
            ...
            paginate.drawOn(cnv, 17.5 * cm, 23.9 * cm)
            ...
            table_header_provider = Table(
                [
                    ...
                    [
                        ...
                        Paragraph(paragraph_over_flow(text=preview_data['provider_zone'],
                                                      width=4,
                                                      font_size=8,
                                                      leading=8),
                                  style_p_header_provider),
                    ...
                    [
                        ...
                        Paragraph(paragraph_over_flow(text=''if preview_data['provider_email'] is None
                        else preview_data['provider_email'],
                                                      width=4,
                                                      font_size=8,
                                                      leading=8),
                                  style_p_header_provider),
                ...
                [1.6 * cm, 4.8 * cm, 2 * cm, 4.6 * cm],
                0.445 * cm,  # Ancho de celdas
            ...
            text_order_n = '<b>Orden Nº</b>' if preview_data['sourceDocumentNumber'] else ''
            ...
            text_trm_value = '{:20,.{}f}'.format(_round(preview_data['exchangeRate'], default_decimals.valueDecimals),
                                                 default_decimals.valueDecimals) \
                if default_decimals.currencyId != preview_data['currencyId'] else ''
            text_currency = '<b>Moneda</b>' if default_decimals.currencyId != preview_data['currencyId'] else ''
            text_currency_value = preview_data['currency'] if default_decimals.currencyId != preview_data['currencyId'] \
                else ''
            ...
            table_header_date_validity = Table(
                [
                    ...
                    [
                        ...
                        Paragraph(text_trm_value, style_p_header_date_validity)
                    ],
                    [
                        Paragraph(text_currency, style_p_header_date_validity),
                        Paragraph(text_currency_value, style_p_header_date_validity)
            ...
            table_header_date_validity.drawOn(cnv, 14 * cm, 21.07 * cm)
            ...
            order_no = '<b>Remisión Nº{0}</b> '.format(preview_data['consecutive'])
            ...
            table_header_order_no.wrap(6.6 * cm, 1 * cm)
            table_header_order_no.drawOn(cnv, 14 * cm, 23.14 * cm)
...
class PurchaseRemissionPreview:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        PurchaseRemissionPreview.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
        ...
        for dct_det in preview_data['document_details']:
            ...
            if dct_det.comments is not None and dct_det.comments != '':
            ...
            if dct_det.lot is not None and dct_det.dueDate is not None:
                det_lot = '<br/><font size=6>LOTE: {0} VENCE: {1}</font>'.format(dct_det.lot,
                                                                                 dct_det.dueDate.strftime('%d %b. %Y')
                                                                                 .upper())
            ...
            data.append(
                [
                    ...
                    Paragraph('{0}{1}{2}{3}{4}{5}{6}'.format(dct_det.item.name,
                                                             det_siz_col_brake,
                                                             det_size,
                                                             det_color,
                                                             det_lot,
        ...
        doc.build(
            [Table(
                data,
                [2.3 * cm, 7 * cm, 1.1 * cm, 1.6 * cm, 2 * cm, 1.3 * cm, 1.2 * cm, 3 * cm],
                style=table_style
            )],
