...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = RefundProviderPreviewM.preview_data
        ...
        def footer(cnv):
            ...
            # table_footer_send.wrap(5.96 * cm, 2.65 * cm)
            ...
            table_footer_totaling = Table(
                ...
                [2.875 * cm, 1 * cm, 2.875 * cm],  # Ancho de las columnas
                0.592 * cm,     # Altura de las columnas
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 13.85 * cm, 14.4 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 16.1 * cm)

            # INFORMACIÓN DE LA TABLA DE OBSERVACIONES - FIN ----------------------------------------------------------

            # INFORMACIÓN DE LA TABLA DE TOTAL ESCRITO ----------------------------------------------------------------

            style_son = ParagraphStyle('Normal')
            style_son.fontName = 'Arial'
            style_son.fontSize = 7
            style_son.leading = 7
            style_son.alignment = TA_JUSTIFY
            ...
            data_footer_total_word = [
                [
                    Paragraph(par_total_word, style_son),
                ]
            ]
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 15.4 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_sing.drawOn(cnv, 1 * cm, 14.4 * cm)
...
class RefundProviderPreviewM:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        RefundProviderPreviewM.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
