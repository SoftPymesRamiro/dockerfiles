...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count, current_page):
        ...
        :return:
        """
        preview_data = AdvanceThirdPreviewM.preview_data
        ...
        def footer(cnv):
            ...
            """
            cnv.saveState()
            preview_data = AdvanceThirdPreviewM.preview_data['checkBook'][0]
            ...
            data_footer_sing = [
                [
                  Paragraph("<b>Observaciones:</b> {0}".format(par_comments), style_p_footer_left)
                ],
                [
            ...
            table_footer_sing.wrap(16 * cm, 2.2 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_sing.drawOn(cnv, 2.8 * cm, 15.5 * cm)
        ...
        def header(cnv, current_page, page_count):
            ...
            preview_header = AdvanceThirdPreviewM.preview_data['checkBook'][0]
            # CHEQUE --------------------------------------------------------------------------------------------------
            ...
            if len(preview_data['checkBook']) == 1:
                default_decimals = AdvanceThirdPreviewM.preview_data['checkBook'][0]['ValueDecimals']
            ...
            table_header_data.wrap(16 * cm, 1 * cm)
            table_header_data.drawOn(cnv, 2.8 * cm, 20.5 * cm)
            ...
            if self._pageNumber == 1:
                total_headers = AdvanceThirdPreviewM.preview_data['checkBook']
            ...
            if current_page > 0 or page_count == 1:
                check = AdvanceThirdPreviewM.preview_data['checkBook'][current_page-1]
            ...
            table_header_provider.wrap(16 * cm, 2.8 * cm)
            table_header_provider.drawOn(cnv, 2.8 * cm, 17.7 * cm)

            cnv.line(16 * cm, 19.3 * cm, 18.8 * cm, 19.3 * cm)  # Linea total
...
class AdvanceThirdPreviewM:
    preview_data = None

    @staticmethod
    def make_same_page_preview_pdf(preview_data):
        AdvanceThirdPreviewM.preview_data = preview_data
        default_decimals = AdvanceThirdPreviewM.preview_data['checkBook'][0]['ValueDecimals']
        ...
        data.append([
            ...
            Paragraph("{0} {1} {2}".format(vacio, vacio, vacio), style),
            Paragraph(vacio, style),
            Paragraph(vacio, style),
            Paragraph('{0}'.format(vacio, default_decimals), style_num),
            Paragraph('{0}'.format(vacio, default_decimals), style_num),
            Paragraph('{0}'.format('.', default_decimals), style_num)
