...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = RefundProviderPreview.preview_data
        ...
        table_style_header_date_validity = TableStyle(
            [
                ...
                # ('SPAN', (-2, 0), (-2, -1)),
                # ('SPAN', (1, 1), (2, 1)),  # span para moneda
                ('SPAN', (0, 2), (-1, -1)),  # span para tasa de cambio
                # ('GRID', (0, 0), (-1, -1), 1, (0, 0, 0)),
            ]
        )
        ...
        def footer(cnv):
            ...
            # INFORMACIÓN DE LA TABLA DE DESPACHO - Vacia ---------------------------------------------------------------------
            ...
            table_footer_send.wrap(5.96 * cm, 2.62 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_send.drawOn(cnv, 1 * cm, 5.7 * cm)

            # INFORMACIÓN DE LA TABLA DE DESPACHO - Vacia - FIN ---------------------------------------------------------------
            ...
            style_p_impo = ParagraphStyle('Normal')
            style_p_impo.fontName = 'Arial'
            style_p_impo.fontSize = 7
            style_p_impo.leading = 7
            style_p_impo.alignment = TA_RIGHT
            ...
            data_footer_taxes = [
                ...
                [
                    Paragraph(base_value_par, style_p_impo),
                    Paragraph(iva_percent_par, style_p_impo),
                    Paragraph(ic_percent_par, style_p_impo),
                    Paragraph(tax_value_par, style_p_impo)
                ]
            ]

            table_footer_taxes = Table(
                data_footer_taxes,  # Contenido de la tabla
                [2 * cm, 1 * cm, 1.6 * cm, 2.14 * cm],  # Ancho de las columnas
                [0.5 * cm, 2.12 * cm],  # Alto de las filas
                style=table_style_footer_taxes  # Estilos de la tabla
            )

            # Denota el contenor de la tabla
            table_footer_taxes.wrap(6.74 * cm, 2.62 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_taxes.drawOn(cnv, 7.1 * cm, 5.7 * cm)
            ...
            data_footer_totaling = [
                ...
                [
                    ...
                    # Paragraph('98,05%', style_p_footer_right),
            ...
            table_footer_totaling = Table(
                ...
                [2.875 * cm, 1.075 * cm, 2.8 * cm],  # Ancho de las columnas
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 13.85 * cm, 1 * cm)

            # cnv.line(20.6 * cm, 30 * cm, 20.6 * cm, 3 * cm)  # Linea base
            # cnv.line(7.1 * cm, 30 * cm, 7.1 * cm, 1 * cm)  # Linea base
            ...
            table_footer_sing = Table(
                ...
                [6.1 * cm, 6.75 * cm],  # Ancho de las columnas
                [1.6 * cm],
        ...
        def header(cnv):
            ...
            table_header_date_validity = Table(
                ...
                [1.2 * cm, 2 * cm, 1.2 * cm, 2.35 * cm],
                0.445 * cm,
                style=table_style_header_date_validity
            )
            table_header_date_validity.wrap(6.75 * cm, 0.445 * cm)
            table_header_date_validity.drawOn(cnv, 13.85 * cm, 21.08 * cm)
            ...
            order_no = '<b>DEVOLUCIÓN A PROVEEDOR</b>'
            order_no_value = '<b>Nº {0}</b>'.format(preview_data['consecutive'])
            style_p_header_provider.alignment = TA_CENTER
            table_header_order_no = Table(
                [
                    [
                        Paragraph("<b>{0}<br/>{1}</b>".format(order_no, order_no_value),
                                  style_p_header_provider),
                    ],
                    [
            ...
            table_header_order_no.drawOn(cnv, 13.85 * cm, 22.42 * cm)
...
class RefundProviderPreview:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        RefundProviderPreview.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
        ...
        doc.build(
            [Table(
                data,
                [2.2 * cm, 7 * cm, 1.1 * cm, 1.7 * cm, 2 * cm, 1.3 * cm, 1.2 * cm, 3 * cm],
                style=table_style
            )],
