...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = ClientsRefundPreview.preview_data
        ...
        def footer(cnv):
            ...
            table_footer_taxes = Table(
                ...
                [0.5 * cm, 2.22 * cm],  # Alto de las filas
            ...
            table_footer_taxes.drawOn(cnv, 7.53 * cm, 3.9 * cm)
            ...
            data_footer_totaling = [
                ...
                [
                    Paragraph('<b>-Otros Dsctos</b>', style_p_total),
            ...
            table_footer_totaling = Table(
                ...
                [0.4 * cm, 0.4 * cm, 0.4 * cm, 0.4 * cm, 0.4 * cm, 0.4 * cm,
                 0.4 * cm, 0.42 * cm, 0.5 * cm],
                style=table_style_footer_totaling  # Estilos de la tabla
            )

            # Denota el contenor de la tabla
            table_footer_totaling.wrap(6.53 * cm, 4.2 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 14.06 * cm, 2.9 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 3.9 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 2.9 * cm)
...
class ClientsRefundPreview:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        ClientsRefundPreview.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId,
