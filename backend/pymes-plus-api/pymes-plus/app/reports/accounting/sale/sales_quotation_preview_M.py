...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = SaleQuotationPreviewM.preview_data
        ...
        style_p_footer_centred.fontSize = 7
        style_p_footer_centred.leading = 7
        ...
        style_p_son.alignment = TA_JUSTIFY
        ...
        style_p_header_provider.fontSize = 7.5
        style_p_header_provider.leading = 7.2
        ...
        def footer(cnv):
            ...
            table_footer_send = Table(
                ...
                [0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.1 * cm],
                # [0.5 * cm, 2.62 * cm]
            ...
            table_footer_send.drawOn(cnv, 1 * cm, 17.5 * cm)
            ...
            table_footer_taxes = Table(
                ...
                [0.4 * cm, 1.5 * cm],  # Alto de las filas
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_taxes.drawOn(cnv, 7.53 * cm, 17.5 * cm)
            ...
            table_footer_totaling = Table(
                ...
                [0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.35 * cm, 0.35 * cm,
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 14.06 * cm, 15.9 * cm)
            ...
            data_footer_observations = [
                [
                    Paragraph(par_observations, style_p_son),
                ]
            ]
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 15.9 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 16.9 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_sing.drawOn(cnv, 1 * cm, 14.8 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            company_information.drawOn(cnv, 2.8 * cm, 14.5 * cm)
        ...
        def header(cnv):
                ...
                [  # esta pendiente por valores
                    Paragraph('<b>Actividad Económica {0} Tarifa Renta {1}</b>'.format(
                        paragraph_over_flow(
                            ...
                            leading=10,
                            left_indent=2.8,
                            right_indent=2.8), preview_data['percentage_renta']),
                        style_p_sub_header),
                ],
                [
            ...
            table_header_provider.drawOn(cnv, 1 * cm, 22 * cm)
            ...
            trm = 0
            if preview_data['exchangeRate'] == '' or preview_data['exchangeRate'] is None:
                trm = 0
            else:
                trm = preview_data['exchangeRate']
            ...
            table_header_date_validity = Table(
                [
                    ...
                    [
                        ...
                        Paragraph("<b>Tasa de Cambio</b> {0}".format(_round(trm, 2), 2),
            ...
            table_header_date_validity.drawOn(cnv, 14.06 * cm, 22 * cm)
            ...
            table_header_description.drawOn(cnv, 1 * cm, 21.6 * cm)

            cnv.line(1 * cm, 21.5 * cm, 20.6 * cm, 21.5 * cm)  # Línea de firma proveedor
            ...
            table_header_order_no = Table(
                [
                    [
                        Paragraph("<b>{0} {1}</b>".format(document_type, document_number),
                                  style_p_header_provider),
                    ],
                    [
            ...
            table_header_order_no.drawOn(cnv, 14.06 * cm, 23.6 * cm)
...
class SaleQuotationPreviewM:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        SaleQuotationPreviewM.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId,
        ...
        for dct_det in preview_data['document_details']:
            ...
            if dct_det.sizeId is not None:
                det_size = '<font size=5.5>TALLA: {0} </font>'.format(dct_det.size.code)
            if dct_det.colorId is not None:
                det_color = '<font size=5.5>COLOR: {0} </font>'.format(dct_det.color.name)
                ...
                if dct_det.item.description is None:
                    ...
                    for info in dct_det.item.description:
                        if i <= 50:
                            info_comment += info
                            i += 1
                        det_comment = '<br/><font size=5.5>{0}</font>'.format(info_comment)
            ...
            if dct_det.lot is not None:
                det_lot = '<br/><font size=5.5>LOTE: {0} VENCE: {1}</font>'.format(dct_det.lot,
                                                                                 dct_det.dueDate.strftime(
                                                                                     '%d %b. %Y')
                                                                                 .upper())
            if dct_det.item.serial:
                serials_str = ", ".join(s.serial.serialNumber for s in dct_det.serialDetail.all())
                det_serials = '<br/><font size=5.5>SERIAL Nº: {0}</font>'.format(serials_str)
            ...
            if preview_data['invima'] == '1':
                if dct_det.item.invimaRegister is None and dct_det.item.invimaDueDate is None:
                    ...
                    invima = '<br/><font size=5.5>{0} {1}</font>'.format(
        ...
        doc.build(
            [Table(
                data,
                [2.1 * cm, 5.9 * cm, 1.2 * cm, 1.7 * cm, 2 * cm, 1.3 * cm, 1.2 * cm, 1.2 * cm, 3 * cm],
                style=table_style
            )],
