...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = SaleRemissionPreviewF.preview_data
        ...
        def footer(cnv):
            ...
            table_footer_send = Table(
                ...
                [0.483 * cm, 0.483 * cm, 0.483 * cm, 0.483 * cm, 0.483 * cm, 0.483 * cm, 0.402 * cm],
                # [0.5 * cm, 2.62 * cm]
            ...
            table_footer_send.drawOn(cnv, 1 * cm, 5.4 * cm)
            ...
            data_footer_taxes = [
                [
                    Paragraph('' if preview_data['not_value'] == '1'
                              else new_value, style_p_footer_right),
                    Paragraph('' if preview_data['not_value'] == '1'
                              else '<b>%IVA</b>', style_p_footer_right),
                    Paragraph('' if preview_data['not_value'] == '1'
                              else '<b>%IC</b>', style_p_footer_right),
                    Paragraph('' if preview_data['not_value'] == '1'
                              else '<b>Vr. IMPTO</b>', style_p_footer_right),
                    Paragraph('', style_p_footer_centred)
                ],
                [
                    Paragraph('' if preview_data['not_value'] == '1'
                              else base_value_par, style_p_footer_right),
                    Paragraph('' if preview_data['not_value'] == '1'
                              else iva_percent_par, style_p_footer_right),
                    Paragraph('' if preview_data['not_value'] == '1'
                              else ic_percent_par, style_p_footer_right),
                    Paragraph('' if preview_data['not_value'] == '1'
                              else tax_value_par, style_p_footer_right),
                    Paragraph('', style_p_footer_centred)
                ]
            ]
            ...
            table_footer_taxes = Table(
                ...
                [0.5 * cm, 2.8 * cm],  # Alto de las filas
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_taxes.drawOn(cnv, 7.53 * cm, 5.4 * cm)
            ...
            table_footer_totaling = Table(
                ...
                [0.4 * cm, 0.4 * cm, 0.4 * cm, 0.4 * cm, 0.4 * cm, 0.4 * cm, 0.4 * cm, 0.5 * cm],
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 14.06 * cm, 5.4 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 3.4 * cm)
            ...
            tot_word = '<para><strong>SON:</strong> {0} </para>'.format(
                '' if preview_data['not_value'] == '1'
                else total_to_letter(_round(var_total_letter, default_decimals.valueDecimals),
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 4.4 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_sing.drawOn(cnv, 1 * cm, 1.8 * cm)

            company_information = Paragraph('Esta Remisión se asimila en todos sus efectos a la letra de cambio '
                                            '(Art. 774 Código de Comercio) '
                                            '<br/>(*)El IVA se cobra en la factura de Remisión '
...
class SaleRemissionPreviewF:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        SaleRemissionPreviewF.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId,
