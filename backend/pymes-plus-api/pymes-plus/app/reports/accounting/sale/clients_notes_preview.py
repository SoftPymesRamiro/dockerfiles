...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = ClientsNotesPreview.preview_data
        ...
        def footer(cnv):
            ...
            table_footer_taxes = Table(
                ...
                [0.5 * cm, 2.62 * cm],  # Alto de las filas
            ...
            table_footer_taxes.drawOn(cnv, 7.53 * cm, 5.7 * cm)
            ...
            table_footer_totaling = Table(
                ...
                [0.4 * cm, 0.4 * cm, 0.5 * cm, 0.4 * cm, 0.4 * cm,
                 0.4 * cm, 0.4 * cm, 0.62 * cm, 0.6 * cm],
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 14.06 * cm, 4.7 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 5.7 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 4.7 * cm)

            # INFORMACIÓN DE LA TABLA DE TOTAL ESCRITO - FIN ----------------------------------------------------------

            # INFORMACIÓN DEL FIRMA -----------------------------------------------------------------------------------

            company_firma = Paragraph('Empresa<br/>Firma y Sello', style_p_footer_centred)
            company_firma.wrap(6 * cm, 0.8 * cm)
            company_firma.drawOn(cnv, 2.8 * cm, 2.3 * cm)

            cnv.line(3 * cm, 3 * cm, 9 * cm, 3 * cm)

            customer_firma = Paragraph('Cliente<br/>Firma y Sello', style_p_footer_centred)
            customer_firma.wrap(6 * cm, 0.8 * cm)
            customer_firma.drawOn(cnv, 12.5 * cm, 2.3 * cm)

            cnv.line(12.5 * cm, 3 * cm, 18.5 * cm, 3 * cm)

            company_information = Paragraph('Contabilidad Pymes+ NIT 830.506.365-7 '
                                            'Teléfono: (572) 3828300 Cali. '
                                            'www.softpymes.com.co', style_p_footer_centred)

            # Denota el contenor de la tabla
            company_information.wrap(16 * cm, 0.8 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            company_information.drawOn(cnv, 2.8 * cm, 0.7 * cm)
...
class ClientsNotesPreview:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        ClientsNotesPreview.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId,
