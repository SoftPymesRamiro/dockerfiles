...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = SaleOrderPreviewM.preview_data
        ...
        def footer(cnv):
            ...
            if preview_data['valorizar'] == '0':
                ...
                # linea total
                cnv.line(17.9 * cm, 16.4 * cm, 20.6 * cm, 16.4 * cm)
            ...
            table_footer_totaling = Table(
                ...
                [0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.5 * cm],
            ...
            table_footer_totaling.drawOn(cnv, 13.7 * cm, 16 * cm)
            ...
            observaciones = '<para><b>Observaciones:</b> {0} </para>'.format(
                                                                        preview_data['comments'].capitalize())
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 17.1 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 16 * cm)
        ...
        def header(cnv):
            ...
            company_name = '{0} - {1}'.format(preview_data['company_name'].upper(), preview_data['branch_name'].upper())
...
class SaleOrderPreviewM:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        SaleOrderPreviewM.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.currencyId,
                                         DefaultValue.branchId,
