...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = SaleInvoiceThirdPartyPreviewT.preview_data
        ...
        style_p_header_date_validity_right.fontSize = 7.5
        ...
        def header(cnv):
                        ...
                        Paragraph('<b>{0}</b>'.format(
                            paragraph_over_flow(
                                text='Teléfonos: {0} {1} {2} {3}'.format(preview_data['phone1'],
                                                                         preview_data['phone2'],
                                                                         preview_data['phone3'],
                                                                         '' if preview_data['fax'] is None
                                                                         or preview_data['fax'] == ''
                                                                         else 'Celular: {0}'.format(
                                                                                                preview_data['fax'])),
                                width=12.8,
                                font_size=12,
                                leading=12)),
            ...
            if preview_data['currencyId'] != default_decimals.currencyId:
                ...
                table_header_provider = Table(
                    [
                        ...
                        [
                            Paragraph(paragraph_over_flow(text='{0}, {1}'.format(
                                                                            preview_data['customer_city'],
            ...
            if preview_data['currencyId'] != default_decimals.currencyId:
                ...
                table_header_description.drawOn(cnv, 1 * cm, 20.7 * cm)
...
class SaleInvoiceThirdPartyPreviewT:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        SaleInvoiceThirdPartyPreviewT.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.currencyId,
                                         DefaultValue.branchId,
        ...
        if preview_data['type_resolution'] == 0 and preview_data['months'] == 0:
            ...
            information_dian = 'Numeración Autorizada por la DIAN según la ' \
                               'Resolución N° {0} de {1} ' \
        ...
        #habilitada
            ...
            information_dian = 'Numeración Habilitada por la DIAN según la ' \
                               'Resolución N° {0} de {1} ' \
                               'desde {2} {3} hasta {4} {5}'.format(
                                                                preview_data['resolution'],
                                                                preview_data['resolution_date'],
        ...
        for dct_det in preview_data['document_details']:
            ...
            if preview_data['currencyId'] == default_decimals.currencyId:
                ...
                style_extra = ParagraphStyle('Normal')
                style_extra_num = ParagraphStyle('Normal')

                style_extra.fontName = 'Arial'
                style_extra.fontSize = 8
                style_extra.leading = 9

                style_extra_num.fontName = 'Arial'
                style_extra_num.fontSize = 8
                style_extra_num.alignment = TA_RIGHT
                # for i in range(40):
                data.append(
                    [
                        Paragraph(dct_det.item.code, style_extra),
                        Paragraph('{0}{1}{2}{3}{4}{5}{6}{7}'.format(dct_det.item.name,
                                                                    ...
                                                                    det_comment,
                                                                    invima), style_extra),
                        Paragraph(dct_det.item.measurementUnit.code, style_extra),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.quantity, default_decimals.quantityDecimals),
                                                      default_decimals.quantityDecimals), style=style_extra_num),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.unitValue, 2), 2), style=style_extra_num),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.value, default_decimals.valueDecimals),
                                                      default_decimals.valueDecimals), style=style_extra_num)
                    ]
                )
