...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = SaleRemissionPreviewM.preview_data
        ...
        style_p_son.leading = 6.8

        style_p_son_center = ParagraphStyle('Normal')
        style_p_son_center.fontName = 'Arial'
        style_p_son_center.fontSize = 6.5
        style_p_son_center.leading = 6.8
        style_p_son_center.alignment = TA_CENTER
        ...
        style_p_footer_centred_text = ParagraphStyle('Normal')
        style_p_footer_centred_text.fontName = 'Arial'
        style_p_footer_centred_text.fontSize = 7
        style_p_footer_centred_text.leading = 7
        style_p_footer_centred_text.alignment = TA_CENTER
        ...
        def footer(cnv):
            ...
            table_footer_totaling = Table(
                ...
                [2.46 * cm, 1.2 * cm, 2.87 * cm],  # Ancho de las columnas
                [0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.4 * cm, 0.5 * cm],
            ...
            table_footer_totaling.drawOn(cnv, 14.06 * cm, 15.5 * cm)
            ...
            # Denota el contenor de la tabla
            table_footer_observations.wrap(4.353 * cm, 1.6 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 15.5 * cm)
            ...
            tot_word = '<para><strong>SON:</strong> {0} </para>'.format(
                '' if preview_data['not_value'] == '1' else
            ...
            table_footer_sing = Table(
                ...
                [4.353 * cm, 4.353 * cm],  # Ancho de las columnas
                [1.7 * cm],
                style=table_style_footer_sing  # Estilos de la tabla
            )

            # Denota el contenor de la tabla
            table_footer_sing.wrap(8.706 * cm, 2.6 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_sing.drawOn(cnv, 5.353 * cm, 15.5 * cm)

            company_information = Paragraph('Esta Remisión se asimila en todos sus efectos a la letra de cambio '
                                            '(Art. 774 Código de Comercio) (*)El IVA se cobra en la factura de Remisión '
                                            '<br/>Contabilidad Pymes+ NIT 830.506.365-7 Teléfono: (572)3828300 Cali. '
            ...
            # Dibuja la tabla en las coordenadas indicadas
            company_information.drawOn(cnv, 2.8 * cm, 15 * cm)
        ...
        def header(cnv):
            ...
            if preview_data['business_agent'] == 'None':
                ...
                vendedor_zone = preview_data['employee_zone']
            else:
                vendedor_name = preview_data['business_agent']
                vendedor_zone = preview_data['business_agent_zone']
            ...
            table_header_provider = Table(
                [
                    ...
                    [
                        ...
                        Paragraph(paragraph_over_flow(text=vendedor_zone,
                                                      width=4.1,
                                                      font_size=8,
                                                      leading=8),
                                  style_p_header_provider),
            ...
            table_header_date_validity = Table(
                [
                    ...
                    [
                        ...
                        Paragraph('<b>Orden N°</b> {0}'.format(preview_data['order_number']),
            ...
            table_header_description = Table(
                [
                    [
                        ...
                        Paragraph('' if preview_data['not_value'] == '1'
...
class SaleRemissionPreviewM:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        SaleRemissionPreviewM.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId,
