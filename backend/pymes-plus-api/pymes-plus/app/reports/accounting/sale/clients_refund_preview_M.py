...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = ClientsRefundPreviewM.preview_data
        ...
        def footer(cnv):
            ...
            if page_count == self._pageNumber:
                ...
                for iva_l in iva_gruop:
                    ...
                    iva_values.append({'iva': value,
                                       'iva_percent': iva_percent,
                                       'base_value': base_value})

                tax_values = iva_values
                ...
                for tax in tax_values:
                    ...
                    if len(tax_value_arr) < 7:
                        tax_value_arr.append(
                            '{:20,.{}f}'.format(_round(tax['iva'], default_decimals.valueDecimals),
                                                default_decimals.valueDecimals))
            ...
            data_footer_taxes = [
                [
                    ...
                    Paragraph('<b>Vr. IVA</b>', style_p_footer_right),
            ...
            table_footer_taxes = Table(
                ...
                [2.5 * cm, 1.5 * cm, 2.3 * cm, 0.23 * cm],  # Ancho de las columnas
            ...
            table_footer_sing = Table(
                ...
                [6.535 * cm, 6.535 * cm, 6.53 * cm],  # Ancho de las columnas
                [1 * cm],
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_sing.drawOn(cnv, 1 * cm, 14.95 * cm)

            style_p_pymes_centred = ParagraphStyle('Normal')
            style_p_pymes_centred.fontName = 'Arial'
            style_p_pymes_centred.fontSize = 6.5
            style_p_pymes_centred.leading = 6.5
            style_p_pymes_centred.alignment = TA_CENTER

            company_information = Paragraph('Contabilidad Pymes+ NIT 830.506.365-7 '
                                            'Teléfono: (572) 3828300 Cali. www.softpymes.com.co',
                                            style_p_pymes_centred)

            # Denota el contenor de la tabla
            company_information.wrap(16 * cm, 0.8 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            company_information.drawOn(cnv, 2.8 * cm, 14.7 * cm)
        ...
        def header(cnv):
            ...
            text_order_n_value = '{0}'.format(preview_data['sourceDocumentNumber']) \
            ...
            table_header_description = Table(
                ...
                [2 * cm, 5.6 * cm, 1.3 * cm, 2 * cm, 2 * cm, 1.3 * cm, 1.2 * cm, 1.2 * cm, 3 * cm],
                [0.4 * cm],
...
class ClientsRefundPreviewM:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        ClientsRefundPreviewM.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId,
        ...
        for dct_det in preview_data['document_details']:
            ...
            if dct_det.lot is not None:
                det_lot = '<br/><font size=6>LOTE: {0} VENCE: {1}</font>'.format(dct_det.lot,
                                                                                 '' if dct_det.dueDate is None
                                                                                 else dct_det.dueDate.strftime(
                                                                                     '%d %b. %Y')
                                                                                 .upper())
        ...
        doc.build(
            [Table(
                data,
                [2.1 * cm, 5.6 * cm, 1.2 * cm, 2 * cm, 2 * cm, 1.3 * cm, 1.2 * cm, 1.2 * cm, 3 * cm],
                style=table_style
            )],
