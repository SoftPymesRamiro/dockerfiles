...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = SaleQuotationPreviewF.preview_data
        ...
        def footer(cnv):
            ...
            table_footer_send = Table(
                ...
                [0.483 * cm, 0.483 * cm, 0.483 * cm, 0.483 * cm, 0.483 * cm, 0.483 * cm, 0.22 * cm],
                # [0.5 * cm, 2.62 * cm]
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 14.06 * cm, 2.6 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 2.6 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 4.2 * cm)
            ...
            data_footer_sing = [
                [
                    Paragraph('<b>{0}</b>'.format(preview_data['createdBy'].title()), style_p_footer_centred),
        ...
        def header(cnv):
            ...
            #         Paragraph('<b>{0}</b>'.format(
            #             paragraph_over_flow(
            #                 text='{0} {1}'.format(preview_data['regimen_iva'],
            #                                       preview_data['ica_activity']),
            #                 width=19.6,
            #                 font_size=9.5,
            #                 leading=9.5)),
            #             style_p_sub_header_ica)
...
class SaleQuotationPreviewF:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        SaleQuotationPreviewF.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId,
