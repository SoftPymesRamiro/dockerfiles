...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = ClientsNotesPreviewM.preview_data
        ...
        #ESTILO TOTAL
        style_p_total = ParagraphStyle('Normal')
        style_p_total.fontName = 'Arial'
        style_p_total.fontSize = 7
        style_p_total.leading = 7
        ...
        def footer(cnv):
            ...
            style_impto = ParagraphStyle('Normal')
            style_impto.fontName = 'Arial'
            style_impto.fontSize = 7
            style_impto.leading = 7
            style_impto.alignment = TA_RIGHT

            data_footer_taxes = [
                [
                    Paragraph(new_value, style_impto),
                    Paragraph('<b>%IVA</b>', style_impto),
                    Paragraph('<b>%IC</b>', style_impto),
                    Paragraph('<b>Vr. IMPTO</b>', style_impto),
                    Paragraph('', style_impto)
                ],
                [
                    Paragraph(base_value_par, style_impto),
                    Paragraph(iva_percent_par, style_impto),
                    Paragraph(ic_percent_par, style_impto),
                    Paragraph(tax_value_par, style_impto),
                    Paragraph('', style_impto)
                ]
            ]
            ...
            table_footer_taxes.drawOn(cnv, 7.53 * cm, 16.65 * cm)
            ...
            data_footer_totaling = [
                [
                    Paragraph('<b>SUBTOTAL</b>', style_p_total),
                    Paragraph('', style_p_total),
                ...
                [
                    Paragraph('<b>-Descuento</b>', style_p_total),
                    Paragraph('', style_p_total),
                ...
                [
                    Paragraph('<b>-IVA Retenido</b>', style_p_total),
                ...
                [
                    Paragraph('<b>NETO DEVOLUCIÓN</b>', style_p_total),
                    Paragraph('', style_p_total),
            ...
            table_footer_totaling = Table(
                ...
                [2.66 * cm, 1.1 * cm, 2.77 * cm],  # Ancho de las columnas
                [0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm, 0.3 * cm,
                 0.3 * cm, 0.3 * cm, 0.4 * cm, 0.5 * cm],
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_totaling.drawOn(cnv, 14.06 * cm, 15.95 * cm)
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_observations.drawOn(cnv, 1 * cm, 16.65 * cm)
            ...
            data_footer_total_word = [
                [
                    Paragraph(par_total_word, style_observaciones),
                ]
            ]
            ...
            # Dibuja la tabla en las coordenadas indicadas
            table_footer_total_word.drawOn(cnv, 1 * cm, 15.95 * cm)

            # INFORMACIÓN DE LA TABLA DE TOTAL ESCRITO - FIN ----------------------------------------------------------

            # INFORMACIÓN DEL FIRMA -----------------------------------------------------------------------------------

            style_firmas = ParagraphStyle('Normal')
            style_firmas.fontName = 'Arial'
            style_firmas.fontSize = 7
            style_firmas.leading = 7
            style_firmas.alignment = TA_CENTER

            company_firma = Paragraph('Empresa - Firma y Sello', style_firmas)
            company_firma.wrap(6 * cm, 0.8 * cm)
            company_firma.drawOn(cnv, 2.8 * cm, 14.7 * cm)

            cnv.line(3 * cm, 15 * cm, 9 * cm, 15 * cm)

            customer_firma = Paragraph('Cliente - Firma y Sello', style_firmas)
            customer_firma.wrap(6 * cm, 0.8 * cm)
            customer_firma.drawOn(cnv, 12.5 * cm, 14.7 * cm)

            cnv.line(12.5 * cm, 15 * cm, 18.5 * cm, 15 * cm)

            company_information = Paragraph('Contabilidad Pymes+ NIT 830.506.365-7 '
                                            'Teléfono: (572) 3828300 Cali. '
                                            'www.softpymes.com.co', style_firmas)

            # Denota el contenor de la tabla
            company_information.wrap(16 * cm, 0.8 * cm)
            # Dibuja la tabla en las coordenadas indicadas
            company_information.drawOn(cnv, 2.8 * cm, 14.4 * cm)
        ...
        def header(cnv):
            ...
            company_name = '{0} - {1}'.format(preview_data['company_name'],
            ...
            data_header_company = [
                ...
                [
                    Paragraph('<b>{0}</b>'.format(
                        paragraph_over_flow(
                            text='{0}'.format(preview_data['ica_activity']),
                            width=19.6,
                            font_size=9.5,
                            leading=9.5)),
                        style_p_sub_header_ica)
            ...
            if preview_data['business_agent'] == 'None':
                vendedor_name = '' if preview_data['employee'] == 'None' \
                    else preview_data['employee']
            else:
                vendedor_name = '' if preview_data['business_agent'] == 'None' \
                    else preview_data['business_agent']
            ...
            table_header_provider = Table(
                ...
                [0.4 * cm, 0.4 * cm, 0.4 * cm, 0.4 * cm, 0.35 * cm, 0.35 * cm, 0.5 * cm],
            ...
            table_header_date_validity = Table(
                ...
                [1.7 * cm, 2.5 * cm, 1.2 * cm, 1.13 * cm],
                0.4 * cm,
                style=table_style_header_date_validity
            )
            table_header_date_validity.wrap(5.1 * cm, 0.6 * cm)
            table_header_date_validity.drawOn(cnv, 14.06 * cm, 21.07 * cm)
            ...
            if preview_data['document_details'][0].item is not None:
                ...
                table_header_description = Table(
                    ...
                    [2 * cm, 9.6 * cm, 1.3 * cm, 1.3 * cm, 1.2 * cm, 1.2 * cm, 3 * cm],
                    [0.4 * cm],
                    style=table_style_header_description
                )
            elif preview_data['document_details'][0].asset is not None:
                table_header_description = Table(
                    [
                        [
                            Paragraph('<b>CÓDIGO</b>', style_p_header_provider),
            ...
            elif preview_data['sourceDocumentType'] == 'FI':
                table_header_description = Table(
                    [
                        [
                            Paragraph('<b>NOMBRE</b>', style_p_header_provider),
                            Paragraph('<b>DOCUMENTO</b>', style_p_header_provider),
                            Paragraph('<b>CANTIDAD</b>', style_p_header_description_right),
                            Paragraph('<b>VALOR</b>', style_p_header_description_right),
                        ]
                    ],
                    [8.8 * cm, 5.3 * cm, 2.5 * cm, 3 * cm],
                    [0.4 * cm],
                    style=table_style_header_description
                )
                ...
                table_header_description = Table(
                    ...
                    [12.9 * cm, 1.3 * cm, 1.2 * cm, 1.2 * cm, 3 * cm],
                    [0.4 * cm],
                    style=table_style_header_description
                )
            ...
            if preview_data['shortWord'] == 'ND':
                document_type = 'NOTA BÉBITO CLIENTE '
            else:
                document_type = 'NOTA CRÉDITO CLIENTE'
            ...
            table_header_order_no.drawOn(cnv, 14.06 * cm, 22.67 * cm)
...
class ClientsNotesPreviewM:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        ClientsNotesPreviewM.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId,
        ...
        for dct_det in preview_data['document_details']:
            ...
            if dct_det.third is not None:
                name_details = "{0} {1} {2} {3}".format(
                    "" if dct_det.third.tradeName is None
                    ...
                    else dct_det.third.firstName.strip())
            else:
                name_details = ''
            ...
            if preview_data['document_details'][0].item is not None:
                ...
                data.append(
                    [
                        Paragraph(dct_det.comments, style),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.disccount, 2), 2), style=style_num),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.iva, 2), 2), style=style_num),
                        Paragraph('{:20,.{}f}'.format(_round(dct_det.withholdingTax, 2), 2),
                                  style=style_num),
        ...
        if preview_data['document_details'][0].item is not None:
            ...
            doc.build(
                [Table(
                    data,
                    [2.1 * cm, 9.6 * cm, 1.2 * cm, 1.3 * cm, 1.2 * cm, 1.2 * cm, 3 * cm],
                    style=table_style
                )],
