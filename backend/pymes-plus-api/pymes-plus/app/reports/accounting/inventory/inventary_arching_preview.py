...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_footer_n_header(self, page_count):
        ...
        :return:
        """
        preview_data = InventaryArchingPreview.preview_data
        ...
        style_p_comentario = ParagraphStyle('Normal')
        style_p_comentario.fontName = 'Arial'
        style_p_comentario.fontSize = 6.5
        style_p_comentario.leading = 6.5
        style_p_comentario.alignment = TA_JUSTIFY
        ...
        def footer(cnv, last_value= 7):
            ...
            #     [3.4 * cm, 0.5 * cm, 3.1 * cm],  # Ancho de las columnas
            ...
            # # table_footer_totaling.wrap(6.6 * cm, 4.2 * cm)
            ...
            #
            # table_footer_totaling.drawOn(cnv, 13.7 * cm, 1 * cm)  # totales del contrato 1
            # cnv.line(17 * cm, 1.37 * cm, 20.7 * cm, 1.37 * cm)  # Línea de total
            ...
            # # Dibuja la tabla en las coordenadas indicadas
            # table_footer_observations.drawOn(cnv, 1 * cm, 1 * cm) # cobservaciones contrato 4.1
        ...
        def header(cnv):
            ...
            observaciones = '<para> {0} </para>'.format(preview_data['comments'].capitalize())
            par_observations = paragraph_over_flow_height(text=observaciones,
                                                          width=10,
                                                          no_par=3,
                                                          font_size=6,
            ...
            table_header_date_validity = Table(
                [
                    ...
                    [
                        ...
                        Paragraph('<b>Arqueo N° </b>', style_p_header_date_validity),
                        Paragraph(preview_data['consecutive'], style_p_header_date_validity),
                        Paragraph('<b>Observaciones:</b> {0}'.format(par_observations), style_p_comentario),
                ...
                [1.8 * cm, 4.3 * cm, 3 * cm, 4 * cm, 2 * cm, 4.5 * cm],
                0.45 * cm,
                style=table_style_header_date_validity
            )
            table_header_date_validity.wrap(19.6 * cm, 0.6 * cm)
            table_header_date_validity.drawOn(cnv, 1 * cm, 23.3 * cm)
            ...
            table_header_description = Table(
                ...
                [1.8 * cm, 9 * cm, 1.4 * cm, 2.1 * cm, 2.6 * cm, 2.7 * cm],
                [0.4 * cm],
                style=table_style_header_description
            )

            table_header_description.wrap(19.6 * cm, 0.4 * cm)
            table_header_description.drawOn(cnv, 1 * cm, 22.8 * cm)
            #
            cnv.line(1 * cm, 22.8 * cm, 20.6 * cm, 22.8 * cm)  # Línea de firma proveedor
...
class InventaryArchingPreview:
    preview_data = None

    @staticmethod
    def make_preview_pdf(preview_data):
        InventaryArchingPreview.preview_data = preview_data
        default_decimals = session.query(DefaultValue.quantityDecimals,
                                         DefaultValue.valueDecimals,
                                         DefaultValue.branchId) \
            .filter(DefaultValue.branchId == preview_data['branch_id']).first()
        ...
        style_p_observa.alignment = TA_CENTER
        ...
        table_style_footer_total = TableStyle(
            [
                ...
                ('BOTTOMPADDING', (0, 0), (-1, -1), 0.1 * cm),
                ('BOX', (0, 0), (-1, -1), 1, (0, 0, 0)),
                ('VALIGN', (0, 0), (3, 3), 'BOTTOM'),
                ('LINEBEFORE', (1, 0), (1, -1), 1, (0, 0, 0)),  # linea vertical
                ('LINEBEFORE', (2, 0), (2, 3), 1, (0, 0, 0)),  # linea vertical
                # ('LINEABOVE', (2, 3), (3, 3), 1, (0, 0, 0)),  # linea horizontal
                # ('LINEABOVE', (0, 7), (0, -2), 1, (0, 0, 0)),  # linea horizontal
                # ('SPAN', (0, 0), (0, 3)),
                ('SPAN', (0, 0), (0, 3)),  # elaborado
                ('SPAN', (1, 0), (1, 3)),  # autorizado
                # ('GRID', (0, 0), (-1, -1), 1, (0, 0, 0))
            ]
        )
        total_pagar = 0
        for x in preview_data['document_details']:
            canti1 = int(x.quantity)
            canti2 = int(x.balance)
            resul = canti1 - canti2

            if resul < 0:
                total_pagar += (x.unitValue * resul) * -1
            else:
                total_pagar += x.unitValue * resul
        ...
        data_footer_totaling = [
            [
                Paragraph("Elaborado por", style_p_observa),
                Paragraph('Autorizado por', style_p_observa),
                Paragraph('', style_p_footer_right),
                Paragraph('', style_p_footer_right),
                Paragraph('', style_p_footer_right)
            ],
            ...
            [
                Paragraph('', style_p_footer),
                Paragraph('', style_p_footer_right),
                Paragraph('<b>TOTAL</b>', style_p_footer),
                Paragraph('', style_p_footer_right),
                Paragraph('${:20,.{}f}'.format(_round(total_pagar, default_decimals.valueDecimals),
                                               default_decimals.valueDecimals), style_p_footer_right)
        ...
        table_footer_totaling = Table(
            data_footer_totaling,  # Contenido de la tabla
            # [2.86 * cm, 1.28 * cm, 2.75 * cm, 2 * cm],  # Ancho de las columnas
            [6.415 * cm, 6.415 * cm, 2.86 * cm, 1 * cm, 2.97 * cm],
            0.5 * cm,
            style=table_style_footer_total  # Estilos de la tabla
        )

        # TABLA OBSERVACIONES Y TOTAL - FIN ----------------------------------------------------------------
        ...
        for dct_det in preview_data['document_details']:
            ...
            cant = int(dct_det.quantity)
            cant2 = int(dct_det.balance)
            # res = 0
            if (cant - cant2) < 0:
                res = (cant-cant2) * -1
            else:
            ...
            data.append(
                [
                    ...
                    Paragraph('{:20,.{}f}'.format(_round(res, default_decimals.quantityDecimals),
                                                  default_decimals.quantityDecimals), style=style_num),
                    Paragraph('{:20,.{}f}'.format(_round(valor_total, default_decimals.valueDecimals),
                                                  default_decimals.valueDecimals), style=style_num)
                ]
            )
        ...
        doc.build(
            [Table(
                data,
                [1.9 * cm, 9 * cm, 1.3 * cm, 2.1 * cm, 2.6 * cm, 2.7 * cm],
                style=table_style
